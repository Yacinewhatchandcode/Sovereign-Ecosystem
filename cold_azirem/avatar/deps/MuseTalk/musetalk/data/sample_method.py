...
def get_src_idx(drive_idx, T, live_method,landmarks_list,image_shapes,top_k_ratio):
    """
        ...
        - live_method (str): Sampling method, which can be "random" or other methods.
    ...
    """
    if live_method == "random":
        src_idx = random.randint(drive_idx - 5 * T, drive_idx + 5 * T)
    elif live_method == "pose_similarity":
        ...
        try:
            ...
            print(e)
            return None
    elif live_method=="pose_similarity_and_closed_mouth":
        # facial contour
        landmark_start_idx = 0
        landmark_end_idx = 16
        try:
    ...
    elif live_method=="pose_similarity_and_mouth_dissimilarity":
        top_k = int(top_k_ratio*len(landmarks_list))
        try:
            top_k = int(top_k_ratio*len(landmarks_list)) 
        ...
        raise ValueError(f"Unknown live_method: {live_method}")
    return src_idx
