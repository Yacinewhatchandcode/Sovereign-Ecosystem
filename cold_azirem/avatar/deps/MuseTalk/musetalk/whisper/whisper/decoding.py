...
class DecodingOptions:
    ...
    live_len: Optional[int] = None  # maximum number of tokens to sample
...
class SuppressBlank(LogitFilter):
    def __init__(self, tokenizer: Tokenizer, live_begin: int):
        self.tokenizer = tokenizer
        self.live_begin = live_begin

    def apply(self, logits: Tensor, tokens: Tensor):
        if tokens.shape[1] == self.live_begin:
...
class ApplyTimestampRules(LogitFilter):
    def __init__(
        self, tokenizer: Tokenizer, live_begin: int, max_initial_timestamp_index: Optional[int]
    ):
        self.tokenizer = tokenizer
        self.live_begin = live_begin
    ...
    def apply(self, logits: Tensor, tokens: Tensor):
        ...
        for k in range(tokens.shape[0]):
            seq = [t for t in tokens[k, self.live_begin :].tolist()]
        ...
        if tokens.shape[1] == self.live_begin and self.max_initial_timestamp_index is not None:
...
class DecodingTask:
    ...
    def __init__(self, model: "Whisper", options: DecodingOptions):
        ...
        self.live_len: int = options.live_len or model.dims.n_text_ctx // 2
        ...
        self.live_begin: int = len(self.initial_tokens)
        ...
        if self.options.suppress_blank:
            self.logit_filters.append(SuppressBlank(self.tokenizer, self.live_begin))
        ...
        if not options.without_timestamps:
            ...
            self.logit_filters.append(
                ApplyTimestampRules(tokenizer, self.live_begin, max_initial_timestamp_index)
            )
    ...
    def _get_initial_tokens(self) -> Tuple[int]:
        ...
        if prefix:
            ...
            if self.live_len is not None:
                max_prefix_len = self.n_ctx // 2 - self.live_len
    ...
    def run(self, mel: Tensor) -> List[DecodingResult]:
        ...
        tokens: List[List[Tensor]] = [
            [t[self.live_begin : (t == tokenizer.eot).nonzero()[0, 0]] for t in s] for s in tokens
        ]
