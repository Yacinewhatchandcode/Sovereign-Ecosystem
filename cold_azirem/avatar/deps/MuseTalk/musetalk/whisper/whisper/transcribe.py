...
from .audio import LIVE_RATE, N_FRAMES, HOP_LENGTH, pad_or_trim, log_mel_spectrogram
...
def transcribe(
    ...
    with tqdm.tqdm(total=num_frames, unit='frames', disable=verbose is not False) as pbar:
        while seek < num_frames:
            # seek是开始的帧数
            end_seek = min(seek + live_skip, num_frames)
            segment = pad_or_trim(mel[:,seek:seek+live_skip], N_FRAMES).to(model.device).to(dtype)
