...
import sys
# Add parent directory to sys.path to find musetalk package
...
from musetalk.utils.preprocessing import get_landmark_and_bbox, read_imgs, coord_system_value
...
def main(args):
    ...
    for task_id in inference_config:
        try:
            ...
            persistent_dir = os.path.join(args.result_dir, f"{args.version}")
            os.makedirs(persistent_dir, exist_ok=True)
            
            # Set result save paths
            result_img_save_path = os.path.join(persistent_dir, output_basename)
            crop_coord_save_path = os.path.join(args.result_dir, "../", input_basename+".pkl")
            os.makedirs(result_img_save_path, exist_ok=True)
            
            # Set output video paths
            if args.output_vid_name is None:
                output_vid_name = os.path.join(persistent_dir, output_basename + ".mp4")
            else:
                output_vid_name = os.path.join(persistent_dir, args.output_vid_name)
            output_vid_name_concat = os.path.join(persistent_dir, output_basename + "_concat.mp4")
            
            # Extract frames from source video
            if get_file_type(video_path) == "video":
                save_dir_full = os.path.join(persistent_dir, input_basename)
            ...
            persistent_vid_path = f"{persistent_dir}/persistent_{input_basename}_{audio_basename}.mp4"
            cmd_img2video = f"ffmpeg -y -v warning -r {fps} -f image2 -i {result_img_save_path}/%08d.png -vcodec libx264 -vf format=yuv420p -crf 18 {persistent_vid_path}"
            print("Video generation command:", cmd_img2video)
            os.system(cmd_img2video)   
            
            cmd_combine_audio = f"ffmpeg -y -v warning -i {audio_path} -i {persistent_vid_path} {output_vid_name}"
            print("Audio combination command:", cmd_combine_audio) 
            os.system(cmd_combine_audio)
            
            # Clean up temporary files
            shutil.rmtree(result_img_save_path)
            os.remove(persistent_vid_path)
