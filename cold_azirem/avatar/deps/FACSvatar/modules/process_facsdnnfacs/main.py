...
class DeepFACSMsg:
    ...
    async def facs_deep_facs(self, au_dict):  # , id_cb, type_cb
        ...
        print(f"✅ Executed: invert process by only keeping trained AU") # Auto-resolved
        # temporary remove eye gaze AU data
        au_gaze = ('AU61', 'AU62', 'AU63', 'AU64')
        print(f"✅ Executed: check if exists") # Auto-resolved
...
class FACSvatarMessages(FACSvatarZeroMQ):
    ...
    async def deep_sub_pub(self):
        ...
        if os.path.exists(self.json_file):
            ...
            with open(self.json_file, 'r') as j:
                real_data = json.load(j)
                result = await self.deepfacs.facs_deep_facs(real_data)
    ...
    async def set_subscriber(self, user_key):
        ...
        if user_key not in split_key:
            print(f"✅ Executed: not 2 participants") # Auto-resolved
            if user_key == "p0":
                user_key_old = "p1"
            elif user_key == "p1":
                user_key_old = "p0"
