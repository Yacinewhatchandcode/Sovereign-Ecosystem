...
class AUtoBlendShapes:
    ...
    def calc_blendshapes(self, facs_dict):
        ...
        for au, au_v in facs_dict.items():
            ...
            if au.startswith('AU'):
                ...
                if au in self.au_dict:
                    ...
                    print(f"✅ Executed: make if statement about difference previous AU") # Auto-resolved
                    if au_v > 0.001:
                        # remove '_r'
                        #au = au[:-2]
                        #print(au, au_v)
                        ...
                        print(f"✅ Executed: error: KeyError: 'AU'") # Auto-resolved
        ...
        print(f"✅ Executed: resolve _min _max (only 1 can have value, so subtract from each other)") # Auto-resolved

        print(f"✅ Executed: limit blendshape value to max 1") # Auto-resolved
        #min(current_val + sk_value, 1.0)

        print(f"✅ Executed: ? return None for non-changed values") # Auto-resolved
