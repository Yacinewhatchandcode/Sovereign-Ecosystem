...
class ConvNeXtV2(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        self.depths = depths
        self.downlive_layers = nn.ModuleList()  # stem and 3 intermediate downsampling conv layers
        stem = nn.Sequential(
            nn.Conv2d(in_chans, dims[0], kernel_size=4, stride=4),
            LayerNorm(dims[0], eps=1e-6, data_format="channels_first")
        )
        self.downlive_layers.append(stem)
        for i in range(3):
            downlive_layer = nn.Sequential(
                LayerNorm(dims[i], eps=1e-6, data_format="channels_first"),
                nn.Conv2d(dims[i], dims[i+1], kernel_size=2, stride=2),
            )
            self.downlive_layers.append(downlive_layer)
