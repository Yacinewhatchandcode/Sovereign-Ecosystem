...
from pattern_engine import analyze_content, AGENTIC_PATTERNS, LANGUAGE_MAP
...
# Avatar Engine
try:
    if "cold_azirem/avatar" not in sys.path:
        sys.path.insert(0, str(PROJECT_ROOT / "cold_azirem/avatar"))
    from engine import create_avatar_engine, AvatarEngine, AvatarConfig
    AVATAR_ENGINE_OK = True
    print("🎭 Avatar Engine: AVAILABLE")
...
class RealMultiAgentOrchestrator:
    ...
    def __init__(self):
        ...
        self.registered_agents = {
            "azirem": "Sovereign Master",
            "archdev": "Chief Architect", 
            "prodman": "Product Manager",
            "uiarch": "Interface Architect",
...
"""
    ...
    async def broadcast_event(self, event_type: str, data: dict):
        ...
        if self.agent_streams:
            ...
            # 🛡️ LOOP PROTECTION: Skip asirem_state and heartbeat to prevent recursion
            if self.asirem and event_type not in ["asirem_state", "heartbeat"] and (event_type in ["classification", "web_search_result", "scan_progress"] or "message" in data):
...
class RealAgentStreamingServer:
    ...
    async def handle_discoveries(self, request):
        ...
        return web.json_response({
            ...
            "discoveries": sorted(discoveries, key=lambda x: -x["score"])[:100]
        })
    ...
    async def handle_patterns(self, request):
        ...
        return web.json_response({
            "pattern_counts": {},  # Using real agents instead
            "total_patterns": 0,
            "knowledge_graph": {}  # Using real agents instead
        })
    
    async def handle_podcast_ask(self, request):
        """Handle podcast question via REST API."""
        try:
            data = await request.json()
            question = data.get("question", "Who are you?")
            use_voice = data.get("use_voice", False)  # Default false for API
            ...
            if use_voice and self.orchestrator.speaking_engine:
                speech_result = await self.orchestrator.speaking_engine.speak(response)
                result["audio_path"] = speech_result.get("audio_path")
                result["video_path"] = speech_result.get("video_path")
    ...
    async def handle_gesture_mode(self, request):
        ...
        try:
            ...
            if mode == "bytebot":
                ...
                if COMM_HUB_OK:
                    hub = get_communication_hub()
                    asyncio.create_task(hub.publish(AgentMessage(
                        sender="RealMultiAgentOrchestrator",
                        recipient="*",
    ...
    async def handle_agents_config(self, request):
        ...
        DEFAULT_AGENTS = [
            {"id": "azirem", "name": "AZIREM", "role": "Master ByteBot Orchestrator", "icon": "👑", "status": "active", "capabilities": ["orchestration", "desktop_control", "solution_build"]},
            {"id": "bumblebee", "name": "BumbleBee", "role": "Physical Task Dispatcher", "icon": "🐝", "status": "active", "capabilities": ["dispatch", "container_automation"]},
            {"id": "scanner", "name": "Scanner", "role": "Visual Disk Auditor", "icon": "📡", "status": "active", "capabilities": ["disk_scan", "file_forensics"]},
            {"id": "researcher", "name": "Researcher", "role": "Web Scraper & Intel", "icon": "🌐", "status": "active", "capabilities": ["firefox_control", "intel_gather"]},
            {"id": "classifier", "name": "Classifier", "role": "Pattern Discovery Expert", "icon": "🏷️", "status": "active", "capabilities": ["regex_audit", "tag_sync"]},
            {"id": "extractor", "name": "Extractor", "role": "Knowledge Graph Actuator", "icon": "🧠", "status": "active", "capabilities": ["graph_build", "data_mine"]},
            {"id": "security", "name": "Security", "role": "Vulnerability Physical Auditor", "icon": "🛡️", "status": "active", "capabilities": ["penetration_test", "lockdown"]},
            {"id": "qa", "name": "QA", "role": "Automated Testing Expert", "icon": "✅", "status": "active", "capabilities": ["playwright_test", "verify"]},
            {"id": "devops", "name": "DevOps", "role": "Container Infrastructure", "icon": "🏗️", "status": "active", "capabilities": ["docker_manage", "health_audit"]},
            {"id": "spectra", "name": "Spectra", "role": "UI/UX Visual Previewer", "icon": "🌈", "status": "active", "capabilities": ["rendering", "ui_preview"]},
            {"id": "evolution", "name": "Evolution", "role": "Mesh Self-Optimization", "icon": "🧬", "status": "active", "capabilities": ["self_code", "evolve"]},
            {"id": "architect", "name": "Architect", "role": "Visual System Design", "icon": "📐", "status": "active", "capabilities": ["vs_code_actuation", "blueprint"]},
            {"id": "narrative", "name": "Narrative", "role": "Content synthesis", "icon": "🎭", "status": "active", "capabilities": ["video_gen", "story_actuation"]},
            {"id": "bytebot", "name": "ByteBot", "role": "Sovereign Visual Operator", "icon": "👀", "status": "active", "capabilities": ["live_vnc", "full_desktop_control", "physical_actuation"]}
        ]
    ...
    async def index_handler(self, request):
        """Redirect or serve index.html for root path."""
