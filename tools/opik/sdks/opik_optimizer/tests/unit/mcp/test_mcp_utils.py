...
def _live_tool_entry() -> dict[str, Any]:
    return {
        "type": "function",
        "function": {
            "name": "doc_lookup",
...
def test_signature_updates_and_tool_conversion() -> None:
    signature = ToolSignature.from_tool_entry(_live_tool_entry())
...
def test_mcp_tool_invocation_cache_behaviour(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def real_call_tool(name: str, payload: dict[str, Any]) -> Any:
        call_count["value"] += 1
    ...
    monkeypatch.setattr(
        ...
        lambda manifest_obj, name, payload: real_call_tool(name, payload),
    )
...
def test_mcp_tool_invocation_cache_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def real_converter(response: Any) -> str:
        call_count["value"] += 1
        return response.content

    monkeypatch.setattr(mcp_workflow, "response_to_text", real_converter)
...
def test_preview_dataset_tool_invocation(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(mcp_workflow, "call_tool_from_manifest", real_call)
...
def test_argument_adapter_resolves_missing_id(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def real_call(name: str, payload: dict[str, Any]) -> Any:
        calls.append((name, payload))
        return Response("/resolved/id")

    prepared = adapter({"library_query": "next"}, real_call)
...
def test_argument_adapter_skips_when_no_queries(
    ...
    prepared = adapter({"topic": "routing"}, real_call)
