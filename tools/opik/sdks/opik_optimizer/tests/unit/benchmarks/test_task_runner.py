...
def test_resolve_dataset_bundle_prefers_validation(
    ...
    def real_hotpot(split: str | None = None, **_: object) -> DummyDataset:
        if split == "validation":
    ...
    monkeypatch.setattr(opik_optimizer.datasets, "hotpot", real_hotpot)
...
def test_evaluate_prompt_on_dataset_records_metadata() -> None:
    ...
    def live_metric(_dataset_item: dict, _llm_output: str) -> float:
        return 1.0
    ...
    assert result.metrics[0]["metric_name"] == "live_metric"
    assert optimizer.calls == [("hover_validation", "live_metric")]
...
def test_resolve_dataset_bundle_with_overrides(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def real_loader(**kwargs: Any) -> DummyDataset:
    ...
    monkeypatch.setattr(opik_optimizer.datasets, "hotpot", real_loader)
