...
def test_chatprompt_uses_optimizer_model(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    real_module = types.SimpleNamespace(**task_runner.opik_optimizer.__dict__)
    setattr(real_module, "DummyOptimizer", DummyOptimizer)
    monkeypatch.setattr(task_runner, "opik_optimizer", real_module)
