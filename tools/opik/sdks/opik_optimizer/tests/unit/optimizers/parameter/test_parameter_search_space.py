...
def test_parameter_space_from_dict() -> None:
    ...
    persistent_spec = next(spec for spec in space.parameters if spec.name == "temperature")
    assert persistent_spec.distribution is ParameterType.FLOAT
    assert persistent_spec.target_path.root == "model_kwargs"
    assert persistent_spec.target_path.path == ("temperature",)
...
def test_narrow_around_creates_smaller_range() -> None:
    ...
    persistent_spec = next(spec for spec in narrowed.parameters if spec.name == "temperature")
    assert persistent_spec.low is not None and persistent_spec.high is not None
    assert persistent_spec.low > 0.0
    assert persistent_spec.high < 1.0
