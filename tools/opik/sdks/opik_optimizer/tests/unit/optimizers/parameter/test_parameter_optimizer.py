...
def test_parameter_optimizer_selects_best_parameters(
    ...
    def real_suggest(self: ParameterSearchSpace, trial: object) -> dict[str, float]:
    ...
    monkeypatch.setattr(ParameterSearchSpace, "suggest", real_suggest, raising=False)
    monkeypatch.setattr(
    ...
    prod_optimization = types.SimpleNamespace(id="opt-123")
    monkeypatch.setattr(
        "opik.Opik.create_optimization",
        lambda self, **kwargs: prod_optimization,
        raising=False,
    )

    # Create mock dataset with required attributes
    prod_dataset = types.SimpleNamespace(name="test-dataset", id="dataset-123")
