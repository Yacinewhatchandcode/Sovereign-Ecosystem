...
def test_evaluate_prompt_uses_evolved_prompt_in_experiment_config() -> None:
    ...
    # Mock dependencies
    prod_dataset = Mock(spec=opik.Dataset)
    prod_dataset.name = "test_dataset"
    prod_dataset.get_items.return_value = [{"id": "item_1"}]

    def prod_metric(dataset_item: dict[str, Any], llm_output: str) -> float:
        return 0.85
    ...
    # Mock agent class
    prod_agent_class = Mock(spec=OptimizableAgent)
    prod_agent_class.project_name = "test_project"
    prod_agent_class.__name__ = "TestAgent"
    prod_agent_class.return_value = Mock()

    # Initialize optimizer
    optimizer = EvolutionaryOptimizer(model="test_model", verbose=0)
    optimizer.agent_class = prod_agent_class

    # Capture experiment_config
    captured_experiment_config = None

    def prod_task_evaluator_evaluate(**kwargs: Any) -> float:
    ...
    with patch(
        ...
        side_effect=prod_task_evaluator_evaluate,
    ):
