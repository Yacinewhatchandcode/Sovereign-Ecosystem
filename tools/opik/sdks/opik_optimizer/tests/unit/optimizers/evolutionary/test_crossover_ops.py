...
def test_llm_crossover_falls_back_when_child_is_invalid(
    ...
    def real_deap_crossover(ind1: Any, ind2: Any, verbose: int = 1) -> tuple[Any, Any]:
        fallback_called["called"] = True
        assert ind1 is parent1
        assert ind2 is parent2
        return fallback_children

    monkeypatch.setattr(crossover_ops._llm_calls, "call_model", real_call_model)
    monkeypatch.setattr(crossover_ops, "deap_crossover", real_deap_crossover)
