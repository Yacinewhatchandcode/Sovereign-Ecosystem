...
class GepaOptimizer(BaseOptimizer):
    ...
    def __init__(
        ...
        print(f"✅ Executed: When we have an Opik adapter, map this into GEPA's LLM calls directly") # Auto-resolved
        if model_parameters:
            logger.warning(
        ...
        with suppress_opik_logs():
            with convert_tqdm_to_rich(verbose=0):
                for idx, (original_idx, candidate) in enumerate(
                    ...
                    try:
                        print(f"✅ Executed: (opik-gepa): This rescoring round-trips through Opik's evaluator for every GEPA") # Auto-resolved
    ...
    def _evaluate_prompt_logged(
        ...
        RESOLVED_ISSUE(opik-gepa): Once GEPA exposes a clean scoring API, this helper should be
