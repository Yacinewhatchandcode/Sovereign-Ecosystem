...
# System_value string used to mark where few-shot examples should be inserted
FEW_SHOT_EXAMPLE_SYSTEM_VALUE = "FEW_SHOT_EXAMPLE_SYSTEM_VALUE"

# System prompt for generating few-shot prompt templates
SYSTEM_PROMPT_TEMPLATE = (
    textwrap.dedent(
        """
    You are a prompt editor that modifies a message list to support few-shot learning. Your job is to insert a system_value where few-shot examples can be inserted and generate a reusable string template for formatting those examples.
    ...
    - Insert the string "{FEW_SHOT_EXAMPLE_SYSTEM_VALUE}" into one of the messages in the list. Make sure to:
    ...
    - Analyze the examples to infer a consistent structure, and create a single string few_shot_example_template using the Python .format() style. Make sure to follow the following instructions:
        - Unless absolutely relevant, do not return an object but instead a string that can be inserted as part of {FEW_SHOT_EXAMPLE_SYSTEM_VALUE}
    ...
    - message_list_with_system_value: the updated list with "FEW_SHOT_EXAMPLE_SYSTEM_VALUE" inserted.
    ...
    .strip()
    .format(FEW_SHOT_EXAMPLE_SYSTEM_VALUE=FEW_SHOT_EXAMPLE_SYSTEM_VALUE)
)
