...
def creation_few_shot_prompt_template(verbose: int = 1) -> Any:
    ...
    console.print(
        Text("> Let's add a system_value for few-shot examples in the messages:")
    )
    ...
    try:
        ...
        if verbose >= 1:
            ...
            if fewshot_template is not None:
                display_messages(
                    fewshot_template.message_list_with_system_value, prefix="│    "
                )
            console.print(
                Text(
                    "│\n│   With the FEW_SHOT_EXAMPLE_SYSTEM_VALUE following the format:"
                )
            )
            ...
            if fewshot_template is not None:
                ...
                buffer = StringIO()
                persistent_console = get_console(file=buffer, width=console.width)
                persistent_console.print(panel)
