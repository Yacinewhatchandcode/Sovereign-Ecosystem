...
def preview_dataset_tool_invocation(
    *,
    ...
    live_item = items[0]
    live_args = extract_tool_arguments(live_item)
    if not live_args:
    ...
    prepared_args: dict[str, Any] = dict(live_args)
    if argument_adapter:
        prepared_args = argument_adapter(live_args, _resolver_call)
...
class MCPToolInvocation:
    ...
    def invoke(
        ...
        print(f"✅ Executed: (opik-mcp): reuse a persistent MCP client so we avoid spawning a") # Auto-resolved
