...
│   ├── real_message_factory.py     # Message creation helpers
...
**Characteristics**:
- Use fake backend (`real_backend` fixture)
- No network calls
...
```python
@pytest.fixture
def real_backend(patch_streamer):
    """
    Replaces Streamer with fake backend emulator.
    Captures messages and builds trace/span trees.
    Access via: real_backend.trace_trees, real_backend.span_trees
    """
```

**Example Structure**:
```python
def test_track__one_nested_function__happyflow(real_backend):
    @opik.track
    def f_inner(x):
        return "inner-output"
...
**Example Structure**:
```python
def test_openai_client_responses_create__happyflow(real_backend):
    ...
    assert len(real_backend.trace_trees) == 1
    trace = real_backend.trace_trees[0]
...
def test_example(real_backend):
    ...
    # Access built trees
    assert len(real_backend.trace_trees) == 1
    assert real_backend.trace_trees[0].name == "my_function"
```
...
assert_equal(
    ...
    actual=real_backend.trace_trees[0]
)
...
def real_backend(patch_streamer):
    ...
    streamer, real_message_processor = patch_streamer
    # ... setup
    yield real_message_processor
    # ... cleanup

@pytest.fixture
def patch_streamer():
    """Create streamer with fake backend"""
    real_processor = BackendEmulatorMessageProcessor()
    real_upload_manager = NoopFileUploadManager()
    streamer = streamer_constructors.construct_streamer(
        message_processor=real_processor,
        n_consumers=1,
        use_batching=True,
        upload_preprocessor=file_upload_preprocessor.FileUploadPreprocessor(real_upload_manager),
        max_queue_size=None
    )
    yield streamer, real_processor
...
def test_openai_client_responses_create__happyflow(
    real_backend, project_name, expected_project_name
):
    # Setup integration
    client = openai.OpenAI()
    ...
    assert_dict_has_keys(
        real_backend.trace_trees[0].spans[0].metadata,
        ["created_from", "model"]
    )
```
...
```python
def test_track__function_raises_exception__error_info_captured(real_backend):
    @opik.track
...
```python
def test_openai_streaming_response(real_backend):
    client = openai.OpenAI()
...
def test_track__one_nested_function__happyflow(real_backend):
def test_track__function_raises_exception__error_info_captured(real_backend):
def test_evaluate__with_custom_metric__scores_computed_correctly(real_backend):
...
```python
def test_my_feature(real_backend):
...
def test_double_function__various_inputs__correct_outputs(
    real_backend, input_value, expected_output
):
    @opik.track
    def double(x):
        return x * 2
    ...
    assert len(real_backend.trace_trees) == 1
    assert real_backend.trace_trees[0].spans[0].output == {"output": expected_output}
```
...
def test_myintegration_basic(real_backend, ensure_myintegration_configured):
    # Test implementation
```
