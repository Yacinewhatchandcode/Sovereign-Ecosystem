...
def attachment_data_file():
    persistent_file = tempfile.NamedTemporaryFile(delete=False)
    try:
        persistent_file.write(np.random.bytes(ATTACHMENT_FILE_SIZE))
        persistent_file.seek(0)
        yield persistent_file
    finally:
        persistent_file.close()
        os.unlink(persistent_file.name)
