...
def test_openai_client_videos_create_and_poll_and_download__happyflow(real_backend):
    """
    ...
    with tempfile.TemporaryDirectory() as persistent_dir:
        output_path = os.path.join(persistent_dir, "test_video.mp4")
    ...
    # Find traces by name
    create_trace = next(
        t for t in real_backend.trace_trees if t.name == "videos.create_and_poll"
    )
    download_trace = next(
        t for t in real_backend.trace_trees if t.name == "videos.download_content"
    )
    write_to_file_trace = next(
        t for t in real_backend.trace_trees if t.name == "videos.write_to_file"
    )
...
def test_openai_client_videos_create_and_poll__error_handling(real_backend):
    """
...
@pytest.mark.asyncio
async def test_openai_async_client_videos_create_and_poll__error_handling(real_backend):
    """
