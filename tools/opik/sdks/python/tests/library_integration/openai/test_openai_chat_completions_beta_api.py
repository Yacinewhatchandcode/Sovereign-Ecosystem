...
def test_async_openai_client_beta_chat_completions_parse__happyflow(real_backend):
...
def test_openai_beta_chat_completion_stream__generator_tracked_correctly(
    ...
    llm_span_metadata = real_backend.trace_trees[0].spans[0].metadata
    _assert_metadata_contains_required_keys(llm_span_metadata)
...
def test_openai_beta_chat_completion_stream__stream_called_2_times__generator_tracked_correctly(
    ...
    assert_equal(EXPECTED_TRACE_TREE_WITH_SHORT_FACT, real_backend.trace_trees[0])
    assert_equal(EXPECTED_TRACE_TREE_WITH_JOKE, real_backend.trace_trees[1])

    llm_fact_span_metadata = real_backend.trace_trees[0].spans[0].metadata
    _assert_metadata_contains_required_keys(llm_fact_span_metadata)

    llm_joke_span_metadata = real_backend.trace_trees[1].spans[0].metadata
