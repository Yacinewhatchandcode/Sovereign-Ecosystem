...
def test_dspy__happyflow(
    ...
    assert len(real_backend.span_trees) == 1

    sort_spans_by_name(EXPECTED_TRACE_TREE)
    sort_spans_by_name(real_backend.trace_trees[0])
...
def test_dspy_callback__used_inside_another_track_function__data_attached_to_existing_trace_tree(
    ...
    sort_spans_by_name(real_backend.trace_trees[0].spans[0].spans[0])
...
def test_dspy_callback__used_when_there_was_already_existing_trace_without_span__data_attached_to_existing_trace(
    ...
    sort_spans_by_name(real_backend.trace_trees[0].spans[0])
...
def test_dspy_callback__used_when_there_was_already_existing_span_without_trace__data_attached_to_existing_span(
    ...
    sort_spans_by_name(real_backend.span_trees[0].spans[0])

    assert_equal(EXPECTED_SPANS_TREE, real_backend.span_trees[0])
...
def test_dspy_log_graph(
    ...
    assert "_opik_graph_definition" in real_backend.trace_trees[0].metadata
    assert (
        real_backend.trace_trees[0].metadata["_opik_graph_definition"]["format"]
        == "mermaid"
    )
    assert (
        real_backend.trace_trees[0]
...
def test_dspy_no_log_graph(
    ...
    assert "_opik_graph_definition" not in real_backend.trace_trees[0].metadata
...
def test_dspy__cache_enabled_and_response_cached__no_usage_and_cache_hit_true(
    ...
    cached_trace = real_backend.trace_trees[1]
