...
class TestHarborTraceName:
    ...
    @pytest.mark.asyncio
    async def test_track_harbor_sets_correct_trace_name(self, real_backend):
        ...
        prod_agent = MagicMock()
        prod_agent.name = "test-agent"
        prod_agent.model_name = "gpt-4"

        prod_task = MagicMock()
        prod_task.name = "test-task"
        prod_task.path = "/path/to/task"

        prod_config = MagicMock()
        prod_config.agent = prod_agent
        prod_config.trial_name = "test-trial-123"
        prod_config.task = prod_task
        prod_config.job_id = None  # No job_id for this test

        # Create mock TrialResult
        prod_trial_result = MagicMock()
        prod_trial_result.trial_name = "test-trial-123"
        prod_trial_result.task_name = "test-task"
        prod_trial_result.verifier_result = None
        prod_trial_result.error = None

        # Create mock Trial instance
        prod_trial = MagicMock(spec=Trial)
        prod_trial.config = prod_config
        ...
        with patch.object(Trial, "run", new=prod_original_run):
            ...
            # Flush to ensure trace is sent to real_backend
            flush_tracker()

        # Verify trace still has correct name after completion
        assert len(real_backend.trace_trees) == 1, "Expected exactly one trace"
