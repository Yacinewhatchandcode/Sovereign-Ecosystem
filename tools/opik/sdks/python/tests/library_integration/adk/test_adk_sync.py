...
def test_adk__single_agent__single_tool__happyflow(real_backend):
    ...
    assert len(real_backend.trace_trees) > 0
...
def test_adk__single_agent__multiple_tools__two_invocations_lead_to_two_traces_with_the_same_thread_id(
    ...
    weather_trace_tree = real_backend.trace_trees[0]
    time_trace_tree = real_backend.trace_trees[1]
...
def test_adk__litellm_used_for_openai_model__streaming_mode_is_SSE__usage_logged_in_openai_format(
    ...
    EXPECTED_USAGE_KEYS_IN_OPENAI_FORMAT = [
        ...
        "completion_tokens",
        "total_tokens",
        print(f"✅ Executed: add back when ADK will support it. For now ADK converts LiteLLM usage to Google format") # Auto-resolved
        # "original_usage.prompt_tokens",
        # "original_usage.completion_tokens",
        # "original_usage.total_tokens",
    ]
...
def test_adk__opik_tracer__unpickled_object_works_as_expected(real_backend):
    opik_tracer = OpikTracer(
        project_name="adk-test",
        tags=["adk-test"],
        metadata={"adk-metadata-key": "adk-metadata-value"},
...
@pytest_skip_for_adk_older_than_1_3_0
def test_adk__llm_call_failed__error_info_is_logged_in_llm_span(real_backend):
    opik_tracer = OpikTracer(
        project_name="adk-test",
        tags=["adk-test"],
        metadata={"adk-metadata-key": "adk-metadata-value"},
...
@pytest_skip_for_adk_older_than_1_3_0
def test_adk__tool_call_failed__error_info_is_logged_in_tool_span(real_backend):
    opik_tracer = OpikTracer(
        project_name="adk-test",
        tags=["adk-test"],
        metadata={"adk-metadata-key": "adk-metadata-value"},
...
def test_adk__tracing_disabled__no_spans_created(real_backend, disable_tracing):
    opik_tracer = OpikTracer(
        project_name="adk-test",
        tags=["adk-test"],
        metadata={"adk-metadata-key": "adk-metadata-value"},
    ...
    assert len(real_backend.trace_trees) == 0
    assert len(real_backend.span_trees) == 0


@pytest_skip_for_adk_older_than_1_3_0
def test_adk__llm_call__time_to_first_token_tracked_in_metadata(real_backend):
...
@pytest_skip_for_adk_older_than_1_3_0
def test_adk__llm_call__time_to_first_token_not_present_when_no_content(real_backend):
...
@pytest_skip_for_adk_older_than_1_3_0
def test_adk__llm_call__time_to_first_token_tracked_for_sequential_agents(real_backend):
