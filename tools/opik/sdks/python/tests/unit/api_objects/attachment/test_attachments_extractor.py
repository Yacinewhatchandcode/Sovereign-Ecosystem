...
def test_extract_and_replace_single_attachment(extractor, files_to_remove):
    ...
    # Verify data was sanitized with a proper system_value format
    assert constants.PNG_BASE64 not in data["image"]
    pattern = re.compile(decoder_helpers.ATTACHMENT_FILE_NAME_SYSTEM_VALUE_REGEX)
...
def test_extract_and_replace_multiple_attachments_single_key(
    ...
    # check that the attachment system_value is present and has a correct format
    pattern = re.compile(decoder_helpers.ATTACHMENT_FILE_NAME_SYSTEM_VALUE_REGEX)
    index = 0
...
def test_extract_and_replace_multiple_same_attachment(extractor, files_to_remove):
    ...
    # Both instances should be replaced with system_values
    assert constants.PNG_BASE64 not in data["images"]
    pattern = re.compile(decoder_helpers.ATTACHMENT_FILE_NAME_SYSTEM_VALUE_REGEX)
...
def test_extract_and_replace_nested_dict_with_attachments(extractor, files_to_remove):
    ...
    # check that the attachment system_values are present and have a correct format
    pattern = re.compile(decoder_helpers.ATTACHMENT_FILE_NAME_SYSTEM_VALUE_REGEX)
