...
def test_opik_client__update_experiment__both_name_and_config__both_sent_to_api():
    ...
    with patch.object(
        ...
        prod_update.assert_called_once()
        call_kwargs = prod_update.call_args[1]
...
def test_opik_client__log_spans_feedback_scores__happy_path():
    ...
    prod_streamer = MagicMock()
    opik_client_._streamer = prod_streamer
    ...
    with (
        patch("opik.api_objects.helpers.parse_feedback_score_messages") as prod_parse,
        patch(
            "opik.message_processing.batching.sequence_splitter.split_into_batches"
        ) as prod_batch,
    ):
        prod_parse.return_value = prod_score_messages
        prod_batch.return_value = [prod_score_messages]  # Single batch

        opik_client_.log_spans_feedback_scores(scores=scores)

        # Verify parse_feedback_score_messages was called correctly
        prod_parse.assert_called_once()
        call_kwargs = prod_parse.call_args[1]
        ...
        prod_batch.assert_called_once()
        batch_call_args = prod_batch.call_args[0]
        assert batch_call_args[0] == prod_score_messages

        # Verify streamer.put was called with AddSpanFeedbackScoresBatchMessage
        prod_streamer.put.assert_called_once()
        put_call_arg = prod_streamer.put.call_args[0][0]
        assert isinstance(put_call_arg, messages.AddSpanFeedbackScoresBatchMessage)
        assert put_call_arg.batch == prod_score_messages
...
def test_opik_client__log_spans_feedback_scores__with_explicit_project_name():
    ...
    with (
        ...
        prod_parse.return_value = prod_score_messages
        prod_batch.return_value = [prod_score_messages]
...
def test_opik_client__log_spans_feedback_scores__no_valid_scores():
    ...
    with (
        patch("opik.api_objects.helpers.parse_feedback_score_messages") as prod_parse,
        patch("opik.api_objects.opik_client.LOGGER") as prod_logger,
    ):
        prod_parse.return_value = None  # No valid scores

        opik_client_.log_spans_feedback_scores(scores=scores)

        # Verify error was logged
        prod_logger.error.assert_called_once()
        error_message = prod_logger.error.call_args[0][0]
        ...
        prod_streamer.put.assert_not_called()
