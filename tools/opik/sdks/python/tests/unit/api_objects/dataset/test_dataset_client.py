...
def test_insert_deduplication__two_dicts_passed_with_the_same_content__only_one_is_inserted():
    ...
    dataset = Dataset("test_dataset", "Test description", prod_rest_client)
    ...
    assert (
        prod_rest_client.datasets.create_or_update_dataset_items.call_count == 1
    ), "create_or_update_dataset_items should be called only once"

    call_args = prod_rest_client.datasets.create_or_update_dataset_items.call_args
...
def test_update__happyflow():
    ...
    insert_call_args = (
        prod_rest_client.datasets.create_or_update_dataset_items.call_args
    )
    ...
    assert (
        prod_rest_client.datasets.create_or_update_dataset_items.call_count == 2
