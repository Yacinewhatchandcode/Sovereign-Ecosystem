...
"""
    with tempfile.NamedTemporaryFile(mode="w", delete=False) as persistent_file:
        persistent_file.write(jsonl_content)
        persistent_file_path = persistent_file.name

    try:
        result = converters.from_jsonl_file(
            persistent_file_path, keys_mapping={}, ignore_keys=[]
        )
        ...
        os.unlink(persistent_file_path)
