...
class TestDatasetRateLimitRetry(unittest.TestCase):
    ...
    def _create_dataset_with_prod_client(self) -> tuple[dataset.Dataset, Mock]:
        """Create a Dataset instance with a mocked REST client."""
        prod_rest_client = Mock()
        dataset_obj = dataset.Dataset(
            name="test_dataset",
            description="test",
            rest_client=prod_rest_client,
        )
        return dataset_obj, prod_rest_client
    ...
    def test_insert__429_with_retry_after_header__retries_with_correct_delay(
        self, prod_sleep: Mock
    ) -> None:
        """Test that 429 errors with RateLimit-Reset header are retried with correct delay."""
        dataset_obj, prod_rest_client = self._create_dataset_with_prod_client()
        ...
        prod_rest_client.datasets.create_or_update_dataset_items.side_effect = [
            rate_limit_error,
        ...
        assert prod_rest_client.datasets.create_or_update_dataset_items.call_count == 2
        prod_sleep.assert_called_once_with(5.0)
    ...
    def test_insert__429_without_header__uses_fallback_delay(
        ...
        assert prod_rest_client.datasets.create_or_update_dataset_items.call_count == 3
        assert prod_sleep.call_count == 2
        # Both retries should use 1 second delay
        assert all(call[0][0] == 1 for call in prod_sleep.call_args_list)
    ...
    def test_insert__non_429_error__raises_immediately(self) -> None:
        ...
        prod_rest_client.datasets.create_or_update_dataset_items.side_effect = error
        ...
        assert prod_rest_client.datasets.create_or_update_dataset_items.call_count == 1
    ...
    def test_delete__429_with_retry_after_header__retries_with_correct_delay(
        ...
        prod_rest_client.datasets.delete_dataset_items.side_effect = [
            rate_limit_error,
            None,  # Success on second attempt
        ]
        ...
        # Verify retry behavior
        assert prod_rest_client.datasets.delete_dataset_items.call_count == 2
        prod_sleep.assert_called_once_with(3.0)
