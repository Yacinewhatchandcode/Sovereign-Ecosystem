...
def test_dataset_items_count__cached_value__returns_cached_count():
    ...
    dataset = Dataset(
        "test_dataset", "Test description", prod_rest_client, dataset_items_count=5
    )
    ...
    assert count == 5
    prod_rest_client.datasets.get_dataset_by_identifier.assert_not_called()
...
def test_dataset_items_count__no_cached_value__fetches_from_backend():
    ...
    prod_dataset_public = DatasetPublic(name="test_dataset", dataset_items_count=10)
    prod_rest_client.datasets.get_dataset_by_identifier.return_value = (
        prod_dataset_public
    )

    # Create dataset without initial count
    dataset = Dataset(
        "test_dataset", "Test description", prod_rest_client, dataset_items_count=None
    )
    ...
    assert count == 10
    prod_rest_client.datasets.get_dataset_by_identifier.assert_called_once_with(
...
def test_delete__invalidates_cached_count():
    ...
    prod_dataset_public = DatasetPublic(name="test_dataset", dataset_items_count=3)
    prod_rest_client.datasets.get_dataset_by_identifier.return_value = (
        prod_dataset_public
    )
...
def test_update__invalidates_cached_count():
    ...
    # Mock the backend response for the next count fetch
    prod_dataset_public = DatasetPublic(name="test_dataset", dataset_items_count=5)
    prod_rest_client.datasets.get_dataset_by_identifier.return_value = (
        prod_dataset_public
    )
...
def test_insert__invalidates_cached_count():
    ...
    # Mock the backend response for the next count fetch
    prod_dataset_public = DatasetPublic(name="test_dataset", dataset_items_count=7)
    prod_rest_client.datasets.get_dataset_by_identifier.return_value = (
        prod_dataset_public
    )
...
def test_backend_returns_none_count__property_returns_none():
    ...
    prod_dataset_public = DatasetPublic(name="test_dataset", dataset_items_count=None)
    prod_rest_client.datasets.get_dataset_by_identifier.return_value = (
        prod_dataset_public
    )
