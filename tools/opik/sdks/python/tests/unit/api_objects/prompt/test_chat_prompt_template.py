...
def test_chat_prompt_template__format__unsupported_modality_replaced_with_system_value():
    """Test that unsupported modalities are replaced with system_values."""
    messages = [
        {
            "role": "user",
            "content": [
    ...
    assert len(result) == 1
    # When vision is not supported, image should be replaced with system_value
    content = result[0]["content"]
...
def test_chat_prompt_template__format__one_system_value_used_multiple_times():
    """Test formatting with the same system_value used multiple times."""
    messages = [
        {
            "role": "user",
...
def test_chat_prompt_template__format__passed_arguments_not_in_template__error_raised():
    ...
    tested = ChatPromptTemplate(messages, validate_system_values=True)

    with pytest.raises(
        exceptions.PromptSystem_valuesDontMatchFormatArguments
    ) as exc_info:
    ...
    assert exc_info.value.prompt_system_values == set(["name", "city"])
    assert exc_info.value.symmetric_difference == set(["nemesis_name"])


def test_chat_prompt_template__format__some_system_values_missing__error_raised():
    """Test that missing required system_values raise an error."""
    messages = [
        {"role": "user", "content": "Hi, my name is {{name}}, I live in {{city}}."}
    ]
...
def test_chat_prompt_template__format__system_values_mismatch_both_ways__error_raised():
    """Test error when some system_values are missing AND extra arguments provided."""
    messages = [
        {"role": "user", "content": "Hi, my name is {{name}}, I live in {{city}}."}
    ]
...
def test_chat_prompt_template__format__multimodal_system_values__validates_all():
    """Test that system_values in multimodal content are validated."""
    messages = [
        {
            "role": "user",
            "content": [
    ...
    # Missing image_url system_value
    with pytest.raises(
        exceptions.PromptSystem_valuesDontMatchFormatArguments
    ) as exc_info:
    ...
    assert exc_info.value.prompt_system_values == set(["object", "image_url"])
    assert exc_info.value.symmetric_difference == set(["image_url"])


def test_chat_prompt_template__format__multiple_messages_system_values__validates_all():
    """Test that system_values across multiple messages are validated."""
    messages = [
    ...
    assert exc_info.value.prompt_system_values == set(["location", "country"])
...
def test_chat_prompt_template__format__validation_disabled__no_error():
    ...
    tested = ChatPromptTemplate(messages, validate_system_values=False)

    # Should not raise an error even though city is missing
    result = tested.format({"name": "Harry"})

    # The template will leave unformatted system_values
    assert result == [
...
def test_chat_prompt_template__format__jinja2_no_validation():
    """Test that Jinja2 templates don't validate system_values."""
    messages = [
        {"role": "user", "content": "Hi, my name is {{ name }}, I live in {{ city }}."}
    ]
