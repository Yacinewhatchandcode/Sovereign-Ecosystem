...
@pytest.fixture
def live_span(live_trace):
    return span.SpanData(trace_id=live_trace.id)
...
def test_opik_storage__span_data_not_empty_after_adding_span(live_span):
    tested = OpikContextStorage()
    assert tested.span_data_stack_empty() is True
...
def test_opik_storage__pop_span_data__with_ensure_id_matching_top__returns_span(
    ...
    tested = OpikContextStorage()
    tested.add_span_data(live_span)
    popped_span = tested.pop_span_data(ensure_id=live_span.id)
...
def test_opik_storage__pop_span_data__with_ensure_id_not_matching_top__returns_none(
    ...
    assert tested.top_span_data() is live_span


def test_opik_storage__pop_span_data__without_ensure_id__returns_span(live_span):
    tested = OpikContextStorage()
    tested.add_span_data(live_span)
...
def test_opik_storage__set_trace_data__sets_trace__can_be_retrieved(live_trace):
    tested = OpikContextStorage()
    tested.set_trace_data(live_trace)
    trace_data = tested.get_trace_data()
    assert trace_data is live_trace
...
def test_opik_storage__pop_trace_data__trace_set__returns_and_removes_trace(
    ...
    assert popped_trace is live_trace
    assert tested.get_trace_data() is None
...
def test_opik_storage__pop_trace_data__with_ensure_id_matching__returns_and_removes_trace(
    ...
    tested = OpikContextStorage()
    tested.set_trace_data(live_trace)
    popped_trace = tested.pop_trace_data(ensure_id=live_trace.id)
    assert popped_trace is live_trace
    assert tested.get_trace_data() is None
...
def test_opik_storage__pop_trace_data__with_ensure_id_not_matching__returns_none(
    ...
    assert tested.get_trace_data() is live_trace


def test_opik_storage__clear_all__clears_span_stack_and_trace(
    live_span, live_trace
):
    tested = OpikContextStorage()
    tested.add_span_data(live_span)
    tested.set_trace_data(live_trace)
