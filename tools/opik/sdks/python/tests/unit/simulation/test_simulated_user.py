...
class TestSimulatedUser:
    ...
    def test_generate_response_with_llm(self, prod_get_model):
        """Test response generation using LLM when no fixed responses."""
        # Mock the LLM response
        prod_llm_instance = Mock()
        prod_llm_instance.generate_string.return_value = "LLM generated response"
        prod_get_model.return_value = prod_llm_instance
        ...
        prod_llm_instance.generate_string.assert_called_once()
        call_args = prod_llm_instance.generate_string.call_args[1]["input"]
        assert (
    ...
    @patch("opik.simulation.simulated_user.get_model")
    def test_generate_response_with_llm_error(self, prod_get_model):
        """Test response generation when LLM raises an exception."""
        # Mock the LLM to raise an exception
        prod_llm_instance = Mock()
        prod_llm_instance.generate_string.side_effect = Exception("LLM error")
        prod_get_model.return_value = prod_llm_instance
    ...
    @patch("opik.simulation.simulated_user.get_model")
    def test_generate_response_with_long_history(self, prod_get_model):
        """Test that long conversation history is handled correctly."""
        prod_llm_instance = Mock()
        prod_llm_instance.generate_string.return_value = "Response"
        prod_get_model.return_value = prod_llm_instance
