...
def test_evaluate__happyflow(
    ...
    prod_dataset.__internal_api__stream_items_as_dataclasses__.assert_called_once()
    ...
    prod_experiment.insert.assert_has_calls(
        [
    ...
    for expected_trace, actual_trace in zip(
        EXPECTED_TRACE_TREES, real_backend.trace_trees
    ):
...
def test_evaluate__exception_raised_from_the_task__error_info_added_to_the_trace(
    ...
    prod_experiment.insert.assert_called_once_with(
    ...
    assert_equal(EXPECTED_TRACE_TREE, real_backend.trace_trees[0])
...
def test_evaluate__with_random_sampler__happy_flow(
    ...
    actual_traces = real_backend.trace_trees
...
def test_evaluate__with_random_sampler__total_items_reflects_sampled_count(
    ...
    prod_dataset.name = "the-dataset-name"
    prod_dataset.dataset_items_count = 10  # Original dataset has 10 items
    # Return 10 items
    prod_dataset.__internal_api__stream_items_as_dataclasses__.return_value = iter(
        [
            dataset_item.DatasetItem(
...
def test_evaluate__with_task_span_metrics__total_items_reflects_actual_count(
    ...
    prod_dataset.name = "the-dataset-name"
    prod_dataset.dataset_items_count = 5
    # Return 5 items
    prod_dataset.__internal_api__stream_items_as_dataclasses__.return_value = iter(
        [
            dataset_item.DatasetItem(
...
def test_evaluate__with_sampler_and_nb_samples__total_items_reflects_final_count(
    ...
    prod_dataset.name = "the-dataset-name"
    prod_dataset.dataset_items_count = 100  # Original dataset has 100 items
    ...
    prod_dataset.__internal_api__stream_items_as_dataclasses__.assert_called_once_with(
        nb_samples=10,
...
def test_evaluate__2_trials_lead_to_2_experiment_items_per_dataset_item(
    ...
    assert len(real_backend.trace_trees) == 4

    # Check that we have 2 traces for each dataset item
    dataset_item_1_traces = [
        trace
        for trace in real_backend.trace_trees
...
def test_evaluate_prompt__2_trials_lead_to_2_experiment_items_per_dataset_item(
    ...
    prod_models_factory_get = mock.Mock()
    prod_model = mock.Mock()
    prod_model.model_name = "some-model-name"
    prod_model.generate_provider_response.return_value = mock.Mock(
        choices=[mock.Mock(message=mock.Mock(content="Hello, world!"))]
    )
    prod_models_factory_get.return_value = prod_model
...
def test_evaluate__with_experiment_scores(real_backend):
    ...
    prod_dataset.name = "test-dataset"
    prod_dataset.dataset_items_count = None
    ...
    prod_rest_client = mock.Mock()
    prod_experiments_api = mock.Mock()
    prod_update_experiment = mock.Mock()
    prod_experiments_api.update_experiment = prod_update_experiment
    prod_rest_client.experiments = prod_experiments_api

    real_experiment = experiment.Experiment(
        id="experiment-id",
        name="test-experiment",
        dataset_name="test-dataset",
        rest_client=prod_rest_client,
        streamer=mock.Mock(),
        experiments_client=mock.Mock(),
    )

    prod_create_experiment = mock.Mock()
    prod_create_experiment.return_value = real_experiment
    ...
    prod_update_experiment.assert_called_once()
    call_args = prod_update_experiment.call_args
...
def test_evaluate__with_experiment_scores_empty_results(real_backend):
    ...
    prod_experiment.name = "test-experiment"
    prod_create_experiment = mock.Mock()
    prod_create_experiment.return_value = prod_experiment
...
def test_evaluate_on_dict_items__happyflow(real_backend):
    items = [
...
def test_evaluate_on_dict_items__with_scoring_key_mapping(real_backend):
    items = [
...
def test_evaluate_on_dict_items__multiple_metrics(real_backend):
    items = [
...
def test_evaluate_on_dict_items__task_execution(real_backend):
    ...
    assert task_calls == [{"value": 5, "expected": 10, "id": "persistent_item_0"}]
...
def test_evaluate_on_dict_items__no_metrics_returns_empty(real_backend):
    items = [{"input": "test"}]
...
def test_evaluate_on_dict_items__empty_items_list(real_backend):
...
def test_evaluate_on_dict_items__task_raises_exception(real_backend):
...
def test_evaluate_on_dict_items__with_scoring_functions(real_backend):
...
def test_evaluate__uses_streaming_by_default(real_backend):
...
def test_evaluate__uses_streaming_with_dataset_item_ids(real_backend):
...
def test_evaluate__falls_back_to_non_streaming_with_dataset_sampler(real_backend):
...
def test_evaluate__streaming_with_nb_samples(real_backend):
