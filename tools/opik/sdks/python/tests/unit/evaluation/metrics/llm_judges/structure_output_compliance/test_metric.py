...
class TestStructuredOutputComplianceMetric:
    ...
    @pytest.fixture
    def structured_output_metric(self, prod_model):
        """Create a StructuredOutputCompliance metric with mocked model."""
        metric = StructuredOutputCompliance(model=prod_model, track=False)
        return metric

    def test_score_basic_compliance(self, structured_output_metric, prod_model):
        ...
        prod_model.generate_string.assert_called_once()
        call_args = prod_model.generate_string.call_args
    ...
    def test_score_with_schema(self, structured_output_metric, prod_model):
    ...
    def test_score_with_few_shot_examples(self, prod_model):
        ...
        metric = StructuredOutputCompliance(
            model=prod_model, few_shot_examples=few_shot_examples, track=False
        )
    ...
    def test_score_model_error_handling(self, structured_output_metric, prod_model):
        """Test error handling when model fails."""
        prod_model.generate_string.side_effect = Exception("Model failed")
    ...
    @pytest.mark.asyncio
    async def test_ascore_async_compliance(self, structured_output_metric, prod_model):
        ...
        # Verify model was called
        prod_model.agenerate_string.assert_called_once()
        call_args = prod_model.agenerate_string.call_args
