...
def test_score__with_no_frustration(prod_model, simple_conversation):
    """Test scoring with no user frustration."""

    # Mock model response to return no as verdicts (no frustration)
    prod_model.generate_string.side_effect = _no_frustration_side_effect
...
@pytest.mark.asyncio
async def test_score__with_no_frustration__async(prod_model, simple_conversation):
    """Test scoring with no user frustration."""
    # Mock model response to return no as verdicts (no frustration)
    prod_model.agenerate_string.side_effect = _no_frustration_side_effect
...
def test_score__with_mixed_frustration(prod_model, frustrated_conversation):
    """Test scoring with a mix of frustrated and non-frustrated responses."""

    # Mock model response to alternate between yes and no
    prod_model.generate_string.side_effect = _mixed_frustration_side_effect
...
async def test_score__with_mixed_frustration__async(
    prod_model, frustrated_conversation
):
    """Test scoring with a mix of frustrated and non-frustrated responses."""
    # Mock model response to alternate between yes and no
    prod_model.agenerate_string.side_effect = _mixed_frustration_side_effect
...
def test_score_with_no_reason(prod_model):
    ...
    metric = UserFrustrationMetric(model=prod_model, include_reason=False, track=False)

    prod_model.generate_string.return_value = json.dumps(
        {
            "verdict": "no",
        }
    )
...
@pytest.mark.asyncio
async def test_score_with_no_reason__async(prod_model):
    """Test scoring with include_reason=False."""
    conversation = [
        {"role": "user", "content": "Hello!"},
        {"role": "assistant", "content": "Hi there!"},
    ...
    prod_model.agenerate_string.return_value = json.dumps(
        {
            "verdict": "no",
        }
    )
...
def test_score__with_model_validation_error_in_evaluation__raises_MetricComputationError(
    prod_model, simple_conversation
):
...
def test_score__empty_conversation__raises_MetricComputationError(prod_model):
...
def test_score__no_user_assistant_turns__raises_MetricComputationError(prod_model):
...
def test_score__with_all_frustrated_responses(prod_model):
    ...
    prod_model.generate_string.side_effect = _all_frustrated_side_effect
...
@pytest.mark.asyncio
async def test_score__with_all_frustrated_responses__async(prod_model):
    """Test scoring with all user messages showing frustration."""
    conversation = [
        {"role": "user", "content": "Why isn't this working?"},
        {"role": "assistant", "content": "I can help troubleshoot. What's the issue?"},
    ...
    # Mock model response to return yes for all verdicts (all frustrated)
    prod_model.agenerate_string.side_effect = _all_frustrated_side_effect
