...
def _install_litellm_stub(monkeypatch, *, supported_params=None):
    ...
    def prod_track_completion(project_name=None):
        def decorator(func):
...
def test_geval_passes_logprobs_only_when_supported(monkeypatch):
    ...
    @contextmanager
    def real_get_provider_response(model_provider, messages, **kwargs):
    ...
    monkeypatch.setattr(base_model, "get_provider_response", real_get_provider_response)
    ...
    @contextmanager
    def real_response_no_logprobs(model_provider, messages, **kwargs):
        captured["kwargs"] = kwargs
        yield SimpleNamespace(
            choices=[
                SimpleNamespace(
    ...
    monkeypatch.setattr(base_model, "get_provider_response", real_response_no_logprobs)
...
async def test_litellm_chat_model_agenerate_string_supports_dict_choices(monkeypatch):
    ...
    @asynccontextmanager
    async def real_aget_provider_response(model_provider, messages, **kwargs):
    ...
    monkeypatch.setattr(
        base_model, "aget_provider_response", real_aget_provider_response
    )
