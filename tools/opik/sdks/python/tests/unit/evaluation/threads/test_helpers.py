...
def test_log_feedback_scores():
    ...
    prod_client = mock.MagicMock(spec=threads_client.ThreadsClient)
    project_name = "test_project"

    # Call the method
    helpers.log_feedback_scores(results, project_name=project_name, client=prod_client)

    # Verify the log_threads_feedback_scores call
    prod_client.log_threads_feedback_scores.assert_called_once()
    call_args = prod_client.log_threads_feedback_scores.call_args[1]
...
def test_load_conversation_thread():
    ...
    prod_opik_client = mock.MagicMock()
    prod_opik_client.search_traces.return_value = prod_traces
    ...
    prod_opik_client.search_traces.assert_called_once_with(
        project_name=project_name,
