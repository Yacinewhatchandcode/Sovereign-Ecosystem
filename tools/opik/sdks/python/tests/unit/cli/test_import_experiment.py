...
class TestRecreateExperiment:
    ...
    def prod_client(self) -> Mock:
        ...
        prod_dataset.__internal_api__insert_items_as_dataclasses__ = Mock()

        # Mock experiment
        prod_experiment = Mock()
        prod_experiment.insert = Mock()
        prod_experiment.id = "exp-123"

        client.get_or_create_dataset = Mock(return_value=prod_dataset)
        client.create_experiment = Mock(return_value=prod_experiment)
    ...
    def test_recreate_experiment_requires_trace_id_map(
        self, prod_client: Mock, experiment_data: ExperimentData
    ) -> None:
        ...
        with patch("opik.cli.imports.experiment.id_helpers_module") as prod_id_helpers:
            prod_id_helpers.generate_id = Mock(return_value="generated-id")
            ...
            assert prod_client._rest_client.experiments.create_experiment_items.called
    ...
    def test_recreate_experiment_batches_dataset_items(
        ...
        with patch("opik.cli.imports.experiment.id_helpers_module") as prod_id_helpers:
            prod_id_helpers.generate_id = Mock(side_effect=["exp-item-1", "exp-item-2"])
            ...
            assert (
                prod_client._rest_client.experiments.create_experiment_items.call_count
                == 1
            )

            # Verify it was called with a list of items (batch)
            call_args = (
                prod_client._rest_client.experiments.create_experiment_items.call_args
            )
    ...
    def test_recreate_experiment_uses_module_names_correctly(
        ...
        with patch("opik.cli.imports.experiment.id_helpers_module") as prod_id_helpers:
            ...
            assert prod_id_helpers.generate_id.called
    ...
    def test_recreate_experiment_handles_empty_trace_id_map(
        ...
        assert prod_client.get_or_create_dataset.called
        assert prod_client.create_experiment.called
    ...
    def test_recreate_experiment_dry_run(
        ...
        assert not prod_client.get_or_create_dataset.called
        assert not prod_client.create_experiment.called
...
class TestImportTracesWithSpans:
    ...
    def prod_client(self) -> Mock:
        ...
        # Mock trace creation
        prod_trace = Mock()
        prod_trace.id = "new-trace-1"
        client.trace = Mock(return_value=prod_trace)

        # Mock span creation
        prod_spans = []
        for i in range(3):
            prod_span = Mock()
            prod_span.id = f"new-span-{i+1}"
            prod_spans.append(prod_span)

        client.span = Mock(side_effect=prod_spans)

        return client

    def test_import_traces_preserves_span_hierarchy(
        self, prod_client: Mock, tmp_path: Path
    ) -> None:
        ...
        trace_id_map, _ = _import_traces_from_projects_directory(
            prod_client, tmp_path, dry_run=False, debug=False
        )

        # Verify spans were created
        assert prod_client.span.call_count == 3

        # Verify spans were created in correct order (root first, then children)
        span_calls = prod_client.span.call_args_list
        ...
        assert prod_client.trace.called
