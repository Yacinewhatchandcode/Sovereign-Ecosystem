...
def test_track__two_nested_functions__happyflow(real_backend):
    @tracker.track
    def f_inner(z):
        return "inner-output"
...
def test_track__two_traces__happyflow(real_backend):
    @tracker.track
    def f_1(x):
        return "f1-output"
    ...
    assert len(real_backend.trace_trees) == 2

    assert_equal(EXPECTED_TRACE_TREES[0], real_backend.trace_trees[0])
    assert_equal(EXPECTED_TRACE_TREES[1], real_backend.trace_trees[1])
...
def test_track__nested_calls_in_separate_threads__3_traces_in_result(real_backend):
    ...
    assert len(real_backend.trace_trees) == 3
    ...
    trace_backend_outer = [
        trace for trace in real_backend.trace_trees if trace.id == trace_outer.id
    ][0]
...
def test_track__span_and_trace_updated_via_opik_context(real_backend):
    @tracker.track
    def f(x):
...
def test_track__span_and_trace_input_output_updated_via_opik_context(real_backend):
    @tracker.track
    def f(x):
        opik_context.update_current_span(
...
def test_tracker__ignore_list_was_passed__ignored_inputs_are_not_logged(real_backend):
...
def test_track__span_usage_updated__openai_format(real_backend):
    @tracker.track
    def f(x):
        opik_context.update_current_span(
            usage={
...
def test_track__span_updated_with_prompts__happy_flow(real_backend):
    prompts = [
...
def test_track__trace_updated_with_prompts__happy_flow(real_backend):
    prompts = [
        prompt.Prompt.from_fern_prompt_version(
            name="system_prompt",
            prompt_version=rest_api.PromptVersionDetail(
...
def test_track__function_called_with_mutable_input_which_changed_afterward__check_span_and_trace_inputs_are_not_affected(
    ...
    assert len(real_backend.trace_trees) == 1
    trace_tree = real_backend.trace_trees[0]

    assert_equal(EXPECTED_TRACE_TREE, trace_tree)


def test_track__using_distributed_headers__spans_are_created_correctly(real_backend):
    @tracker.track
