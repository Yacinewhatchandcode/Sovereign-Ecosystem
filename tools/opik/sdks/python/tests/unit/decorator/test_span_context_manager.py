...
def test_start_as_current_span__inside__happy_flow(real_backend):
...
def test_start_as_current_span__outside__happy_flow(real_backend):
...
def test_start_as_current_span__mixed__inside_override_outside(real_backend):
...
def test_start_as_current_span__user_error__logged_and_raised(real_backend):
...
def test_start_as_current_span__parent_trace_exists__new_not_created(real_backend):
...
def test_start_as_current_span__context_cleanup__span_and_trace_removed_after_exit(
    ...
    assert len(real_backend.trace_trees) == 1
    assert real_backend.trace_trees[0].name == "test-span"
    assert len(real_backend.trace_trees[0].spans) == 1
    assert real_backend.trace_trees[0].spans[0].name == "test-span"
