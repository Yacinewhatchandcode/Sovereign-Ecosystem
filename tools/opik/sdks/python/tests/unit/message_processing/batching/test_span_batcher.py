...
def test_create_span_message_batcher__exactly_max_batch_size_reached__batch_is_flushed():
    ...
    span_messages = real_message_factory.real_span_create_message_batch(
        count=MAX_BATCH_SIZE, approximate_span_size=real_message_factory.ONE_MEGABYTE
    )
...
def test_create_span_message_batcher__more_than_max_batch_size_items_added__one_batch_flushed__some_data_remains_in_batcher():
    ...
    span_messages = real_message_factory.real_span_create_message_batch(
        ...
        approximate_span_size=real_message_factory.ONE_MEGABYTE,
    )
...
def test_create_span_message_batcher__split_message_into_batches__size_limit_reached():
    ...
    assert batcher.is_empty()
    span_messages = real_message_factory.real_span_create_message_batch(
        count=2 * 2, approximate_span_size=real_message_factory.ONE_MEGABYTE
    )
...
def test_create_span_message_batcher__add_duplicated_span__previous_span_is_removed_from_batcher():
    ...
    span_messages += real_message_factory.real_span_create_message_batch(
        count=1,
        approximate_span_size=real_message_factory.ONE_KILOBYTE,
        has_ended=False,
    )
    span_messages += real_message_factory.real_span_create_message_batch(
        count=1, approximate_span_size=real_message_factory.ONE_KILOBYTE, has_ended=True
    )
