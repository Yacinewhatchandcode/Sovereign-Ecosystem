...
def processor(prod_streamer):
    ...
    return attachments_extraction_processor.AttachmentsExtractionProcessor(
        ...
        messages_streamer=prod_streamer,
...
@pytest.fixture
def inactive_processor(prod_streamer):
...
def test_process_attachment_support_message_with_no_attachments(
    ...
    assert prod_streamer.put.call_count == 1
    requeued_message = prod_streamer.put.call_args[0][0]
...
def test_process_span_message_extracts_from_input(processor, prod_streamer):
    ...
    prod_attachment = attachment.Attachment(
        data="/tmp/image.png",
        file_name="input-attachment-123.png",
        content_type="image/png",
    )

    prod_attachment_with_context = attachment_context.AttachmentWithContext(
        attachment_data=prod_attachment,
        entity_type="span",
        entity_id="span-123",
        project_name="test-project",
        context="input",
    ...
    def prod_extract(data, entity_type, entity_id, project_name, context):
        if context == "input":
            return [prod_attachment_with_context]
        return []

    with mock.patch.object(
        processor.extractor, "extract_and_replace", side_effect=prod_extract
    ):
        processor.process(wrapped_message)

    # Should have created one CreateAttachmentMessage and re-queued original
    assert prod_streamer.put.call_count == 2

    # First call should be the attachment message
    first_call = prod_streamer.put.call_args_list[0][0][0]
    ...
    second_call = prod_streamer.put.call_args_list[1][0][0]
    assert second_call is original_message
    assert hasattr(second_call, constants.MARKER_ATTRIBUTE_NAME)


def test_process_span_message_extracts_from_output(processor, prod_streamer):
    ...
    assert prod_streamer.put.call_count == 2
    attachment_msg = prod_streamer.put.call_args_list[0][0][0]
    assert isinstance(attachment_msg, messages.CreateAttachmentMessage)


def test_process_span_message_extracts_from_metadata(processor, prod_streamer):
...
def test_process_trace_message_extracts_attachments(processor, prod_streamer):
...
def test_process_update_span_message(processor, prod_streamer):
...
def test_process_update_trace_message(processor, prod_streamer):
...
def test_process_multiple_attachments_from_different_contexts(processor, prod_streamer):
    ...
    assert prod_streamer.put.call_count == 4

    # First 3 should be attachment messages
    for i in range(3):
        msg = prod_streamer.put.call_args_list[i][0][0]
        assert isinstance(msg, messages.CreateAttachmentMessage)

    # Last should be the original message
    last_msg = prod_streamer.put.call_args_list[3][0][0]
    assert last_msg is original_message


def test_process_with_null_input_output_metadata(processor, prod_streamer):
    ...
    with mock.patch.object(processor.extractor, "extract_and_replace") as prod_extract:
        processor.process(wrapped_message)

        # Extractor should not be called
        prod_extract.assert_not_called()

    # Only the original message should be re-queued
    assert prod_streamer.put.call_count == 1


def test_process_inactive_processor_skips_extraction(inactive_processor, prod_streamer):
...
def test_process_handles_extraction_exception_gracefully(processor, prod_streamer):
