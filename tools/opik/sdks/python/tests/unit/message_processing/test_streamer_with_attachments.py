...
@pytest.fixture
def streamer_with_attachments_enabled(real_file_upload_preprocessor):
...
@pytest.fixture
def streamer_with_attachments_disabled(real_file_upload_preprocessor):
...
def test_streamer__create_span_message__attachments_enabled__does_not_wrap_without_end_time(
    ...
    tested, prod_message_processor = streamer_with_attachments_enabled
    ...
    processed_message = prod_message_processor.process.call_args[0][0]
...
def test_streamer__create_span_message__attachments_disabled__does_not_wrap(
    ...
    tested, prod_message_processor = streamer_with_attachments_disabled
...
def test_streamer__multiple_span_messages__attachments_enabled__not_wrapped_without_end_time(
    ...
    assert prod_message_processor.process.call_count >= 1

    # Check that none of the messages are wrapped in AttachmentSupportingMessage
    for call in prod_message_processor.process.call_args_list:
