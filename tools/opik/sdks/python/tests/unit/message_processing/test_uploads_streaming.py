...
def test_streamer__attachment_uploads__flush__ok(persistent_file_15mb):
    ...
    with patch("opik.file_upload.upload_manager.file_uploader") as uploader_mock:
        ...
        attachment = messages.CreateAttachmentMessage(
            file_path=persistent_file_15mb.name,
            file_name="test_file",
            mime_type=None,
            entity_type="span",
            entity_id=NOT_USED,
...
def test_streamer__attachment_uploads__flush__timeout(persistent_file_15mb):
    def upload_noop(**kwargs):
        print("Uploading file")
        time.sleep(2)
