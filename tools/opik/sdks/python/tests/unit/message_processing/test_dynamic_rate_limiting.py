...
@pytest.fixture
def streamer_with_prod_message_processor(real_file_upload_preprocessor):
    ...
    yield streamer, prod_message_processor

    streamer.close(None)


def test_dynamic_rate_limiting__rate_limited__check_queue_messages_are_put_back(
    streamer_with_prod_message_processor,
):
    streamer, prod_message_processor = streamer_with_prod_message_processor

    # to allow a few skipped loop iterations
    retry_after = queue_consumer.SLEEP_BETWEEN_LOOP_ITERATIONS * 3
    prod_message_processor.process.side_effect = (
    ...
    prod_message_processor.process = lambda message: None
...
def test_dynamic_rate_limiting__rate_limited__check_streamer_flush_is_blocking_until_all_messages_are_processed(
    ...
    prod_message_processor.process = process
