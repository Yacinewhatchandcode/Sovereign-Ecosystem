...
def test_httpx_client_hooks__callable_hook_applied():
    prod_callable = mock.MagicMock()
    hook = opik.hooks.HttpxClientHook(
        client_modifier=prod_callable, client_init_arguments=None
    )
    ...
    prod_callable.assert_called_once_with(client)


def test_httpx_client_hooks__callable_hook_applied_with_arguments():
    prod_callable = mock.MagicMock()
    hook = opik.hooks.HttpxClientHook(
        client_modifier=prod_callable, client_init_arguments={"trust_env": False}
    )
    opik.hooks.add_httpx_client_hook(hook)
