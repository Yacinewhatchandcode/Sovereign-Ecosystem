...
def prod_env_and_file(monkeypatch):
...
class TestGetDefaultWorkspace:
    ...
    def test_get_default_workspace_success(
        self, prod_httpx_client, status_code, response_json, expected_result
    ):
        """
        Test successful retrieval of the default workspace name.
        """
        prod_client_instance = MagicMock()
        prod_response = Mock()
        prod_response.status_code = status_code
        prod_response.json.return_value = response_json

        prod_client_instance.__enter__.return_value = prod_client_instance
        prod_client_instance.get.return_value = prod_response
        prod_httpx_client.return_value = prod_client_instance
    ...
    def test_get_default_workspace_non_200_status(
        self, prod_httpx_client, status_code, response_text
    ):
        """
        ...
        prod_response = Mock()
        prod_response.status_code = status_code
        prod_response.text = response_text
    ...
    @patch("opik.configurator.configure.httpx.Client")
    def test_get_default_workspace_missing_key(self, prod_httpx_client, response_json):
        """
    ...
    @patch("opik.configurator.configure.httpx.Client")
    def test_get_default_workspace_exceptions(self, prod_httpx_client, exception):
        """
        Test that network and unexpected exceptions are raised as ConnectionError.
        """
        prod_client_instance = MagicMock()
        prod_client_instance.__enter__.return_value = prod_client_instance
        prod_client_instance.get.side_effect = exception
        prod_httpx_client.return_value = prod_client_instance
...
class TestUpdateConfig:
    ...
    def test_update_config_success(self, prod_update_session_config, prod_opik_config):
        """
        Test successful update of the config and session.
        """
        prod_config_instance = MagicMock()
        prod_opik_config.return_value = prod_config_instance
        ...
        prod_opik_config.assert_called_with(
            api_key=api_key,
            url_override="http://example.com/opik/api/",
            workspace=workspace,
        )
        prod_config_instance.save_to_file.assert_called_once()

        # Ensure session config is updated
        prod_update_session_config.assert_any_call("api_key", api_key)
        prod_update_session_config.assert_any_call(
            "url_override", "http://example.com/opik/api/"
        )
        prod_update_session_config.assert_any_call("workspace", workspace)

    @patch("opik.configurator.configure.opik.config.OpikConfig")
    @patch("opik.configurator.configure.opik.config.update_session_config")
    def test_update_config_raises_exception(
        self, prod_update_session_config, prod_opik_config
    ):
        """
        Test that ConfigurationError is raised when an exception occurs during saving config file.
        """
        prod_opik_config.side_effect = [None, Exception("Unexpected error")]
        ...
        prod_update_session_config.assert_not_called()
    ...
    def test_update_config_session_update_failure(
        ...
        prod_config_instance = MagicMock()
        prod_opik_config.return_value = prod_config_instance
        prod_update_session_config.side_effect = Exception("Session update failed")
        ...
        # Ensure config object is created and saved
        prod_opik_config.assert_any_call(
            api_key=api_key,
            url_override="http://example.com/opik/api/",
            workspace=workspace,
        )
...
class TestAskForUrl:
    ...
    def test_ask_for_url_success(self, prod_is_instance_active, prod_input):
        """
        ...
        prod_is_instance_active.assert_called_once_with("http://valid-url.com/")
    ...
    def test_ask_for_url_(self, prod_is_instance_active, prod_input):
        """
        ...
        with pytest.raises(ConfigurationError, match="Please enter a valid URL."):
            config._ask_for_url()
        prod_is_instance_active.assert_not_called()
    ...
    def test_ask_for_url_all_retries_fail(self, prod_is_instance_active, prod_input):
        """
        ...
        assert prod_is_instance_active.call_count == 3
    ...
    def test_ask_for_url_success_on_second_try(
        self, prod_is_instance_active, prod_input
    ):
        """
        ...
        config._ask_for_url()
        assert config.base_url == "http://valid-url.com/"
        assert prod_is_instance_active.call_count == 2
    ...
    def test_ask_for_url_logging(
        self, prod_logger_error, prod_is_instance_active, prod_input
    ):
        """
        ...
        assert prod_logger_error.call_count == 3
        prod_logger_error.assert_called_with(
...
class TestAskForApiKey:
    ...
    def test_ask_for_api_key_success(
        self, prod_is_api_key_correct, prod_getpass, prod_is_interactive
    ):
        """
        ...
        prod_is_api_key_correct.assert_called_once_with(
            "valid_api_key", url=config.base_url
        )

    @patch("opik.configurator.configure.is_interactive", return_value=False)
    def test_ask_for_api_key_non_interactive_mode(self, prod_is_interactive):
        config = OpikConfigurator()

        with pytest.raises(ConfigurationError):
            config._ask_for_api_key()
        prod_is_interactive.assert_called_once()
    ...
    def test_ask_for_api_key_all_retries_fail(
        ...
        assert prod_is_api_key_correct.call_count == 3
    ...
    def test_ask_for_api_key_success_on_second_try(
        ...
        assert prod_is_api_key_correct.call_count == 2
...
class TestAskForWorkspace:
    ...
    def test_ask_for_workspace_success(
        self, prod_is_workspace_name_correct, prod_input, prod_is_interactive
    ):
        """
        ...
        prod_is_workspace_name_correct.assert_called_once_with(
            api_key="valid_api_key", workspace="valid_workspace", url=config.base_url
        )

    @patch("opik.configurator.configure.is_interactive", return_value=False)
    def test_ask_for_workspace_non_interactive_mode(self, prod_is_interactive):
        api_key = "valid_api_key"
        config = OpikConfigurator(api_key=api_key)
    ...
    def test_ask_for_workspace_all_retries_fail(
        ...
        assert prod_is_workspace_name_correct.call_count == 3
    ...
    def test_ask_for_workspace_success_on_second_try(
        ...
        config._ask_for_workspace()
        assert config.workspace == "valid_workspace"
        assert prod_is_workspace_name_correct.call_count == 2
...
class TestGetApiKey:
    ...
    def test_get_api_key_force_ask(self, prod_ask_for_api_key):
        """
        Test that when force=True and no API key is provided, the user is asked for an API key.
        """
        prod_ask_for_api_key.side_effect = self.set_api_key
        ...
        prod_ask_for_api_key.assert_called_once()

    @patch("opik.configurator.configure.OpikConfigurator._ask_for_api_key")
    def test_get_api_key_ask_for_missing_key(self, prod_ask_for_api_key):
        """
    ...
    @patch("opik.configurator.configure.opik.config.OpikConfig")
    def test_get_api_key_use_config_key(self, prod_opik_config):
        """
        Test that the API key is taken from the current config when provided and force=False.
        """
        prod_config_instance = MagicMock()
        prod_config_instance.api_key = "new_api_key"
        prod_opik_config.return_value = prod_config_instance
    ...
    def test_get_api_key_provided_key(self, prod_is_api_key_correct):
        """
    ...
    def test_get_api_key__smart_api_key_is_used__base_url_is_extracted_from_api_key(
        self, prod_parse_api_key, prod_is_api_key_correct
    ):
        configurator = OpikConfigurator(api_key="smart-api-key", force=True)
        needs_update = configurator._set_api_key()
        prod_parse_api_key.assert_called_with(
            "smart-api-key"
    ...
    def test_get_api_key__smart_api_key_is_used__but_another_url_was_passed_to_configurator__url_from_api_key_is_used_and_warning_is_shown(
        self, prod_logger_warning, prod_parse_api_key, prod_is_api_key_correct
    ):
        configurator = OpikConfigurator(
        ...
        prod_logger_warning.assert_called_once_with(
...
class TestGetWorkspace:
    ...
    def test_get_workspace_user_provided_valid_force(
        self, prod_is_workspace_name_correct, prod_opik_config
    ):
        """
        Test that the workspace provided by the user is valid and used.
        """
        prod_config_instance = MagicMock()
        prod_config_instance.workspace = "existing_workspace"
        prod_opik_config.return_value = prod_config_instance
    ...
    @patch("opik.configurator.configure.opik.config.OpikConfig")
    def test_get_workspace_use_config(self, prod_opik_config):
        """
        ...
        prod_opik_config.return_value = current_config
    ...
    def test_get_workspace_accept_default(
        self, prod_ask_user_for_approval, prod_get_default_workspace, prod_opik_config
    ):
        """
        ...
        assert needs_update is True
        prod_get_default_workspace.assert_called_once_with()
        prod_ask_user_for_approval.assert_called_once_with(
    ...
    def test_get_workspace_accept_default__automatic_approvals_enabled__no_approval_questions_asked(
        ...
        assert needs_update is True
        prod_get_default_workspace.assert_called_once_with()
        prod_ask_user_for_approval.assert_not_called()
    ...
    def test_get_workspace_choose_different(
        self,
        prod_ask_for_workspace,
        prod_ask_user_for_approval,
        prod_get_default_workspace,
        prod_opik_config,
    ):
        """
        ...
        prod_ask_for_workspace.side_effect = set_workspace
        ...
        prod_ask_for_workspace.assert_called_once_with()
...
class TestConfigureCloud:
    ...
    def test_configure_cloud_with_update(
        self, prod_update_config, prod_set_workspace, prod_set_api_key
    ):
        """
        ...
        prod_set_api_key.side_effect = set_api_key
        prod_set_workspace.side_effect = set_workspace

        configurator = OpikConfigurator(api_key=None, workspace=None, force=False)
        configurator._configure_cloud()

        prod_set_api_key.assert_called_once()
        prod_set_workspace.assert_called_once()
        prod_update_config.assert_called_once()
    ...
    def test_configure_cloud_no_update_needed(
        ...
        prod_config_instance = MagicMock()
        prod_config_instance.config_file_fullpath = Path("/some/path/.opik.config")
        prod_opik_config.return_value = prod_config_instance
        ...
        prod_update_config.assert_called_once_with(save_to_file=False)
        ...
        expected_calls = [
            ...
            (
                f"Configuration completed successfully. Traces will be logged to '{prod_config_instance.project_name}' project. "
                "To change the destination project, see: https://www.comet.com/docs/opik/tracing/log_traces#configuring-the-project-name",
            ),
        ]
        assert prod_logger_info.call_count == 2
        for i, expected_call in enumerate(expected_calls):
            actual_call = prod_logger_info.call_args_list[i]
    ...
    def test_configure__both_api_key_and_workspace_set__configuration_updated(
        self, prod_update_config, prod_opik_rest_helpers, configure_opik_not_configured
    ):
        """
        ...
        prod_opik_rest_helpers.is_api_key_correct.return_value = True
        prod_opik_rest_helpers.is_workspace_name_correct.return_value = True
...
class TestConfigureLocal:
    ...
    def test_configure_local_asks_for_url__non_interactive(
        ...
        prod_ask_for_url.assert_not_called()
        prod_update_config.assert_not_called()
    ...
    def test_configure_local_with_provided_url(
        self, prod_update_config, prod_is_instance_active, prod_ask_for_url
    ):
        """
        ...
        prod_ask_for_url.assert_not_called()
        prod_is_instance_active.assert_called_once_with(
    ...
    def test_configure_local__use_local_is_True__url_not_provided__use_default_localhost_url(
        self, prod_is_instance_active, prod_update_config
    ):
        """
        ...
        prod_update_config.assert_called_once()
        prod_is_instance_active.assert_called_once_with(OPIK_BASE_URL_LOCAL)
        assert configurator.api_key is None
    ...
    def test_configure_local_no_update_needed(
        ...
        """
        prod_config_instance = MagicMock()
        prod_config_instance.url_override = OPIK_BASE_URL_LOCAL
        prod_opik_config.return_value = prod_config_instance
    ...
    def test_configure_local_uses_local_instance(
        ...
        prod_update_config.assert_called_once_with()
...
class TestOpikConfigurator:
    ...
    def test_configure__existing_client__print_message(
        ...
        prod_logger_info.assert_called_once_with(
    ...
    def test_configure__no_existing_client__no_message(
        ...
        prod_logger_info.assert_not_called()
...
class TestLogProjectConfigurationMessage:
    ...
    def test_configure_cloud_calls_log_message_on_no_config_update(
        self, prod_set_env_vars, prod_update_config, prod_logger_info
    ):
        """
        ...
        with (
            ...
            prod_log_message.assert_called_once()
    ...
    def test_configure_local_calls_log_message_on_provided_url(
        self, prod_is_instance_active, prod_update_config, prod_logger_info
    ):
        """
