...
def test_custom_filter_chain(real_error_event, real_basic_hint):
    count_handler = filter_by_count.FilterByCount(max_count=1, level="error")

    tested_chain = filter_chain.FilterChain([count_handler])

    assert tested_chain.validate(real_error_event, real_basic_hint) is True
    # because of the count = 1 should be false
    assert tested_chain.validate(real_error_event, real_basic_hint) is False
