...
def test_filter_by_count__errors(
    max_count, real_warning_event, real_error_event, real_basic_hint
):
    tested = filter_by_count.FilterByCount(max_count=max_count, level="error")

    for _ in range(max_count):
        assert tested.process_event(real_error_event, real_basic_hint) is True

    assert tested.process_event(real_error_event, real_basic_hint) is False
    # should be neutral about warnings
    assert tested.process_event(real_warning_event, real_basic_hint) is True
...
def test_filter_by_count__warning(
    ...
    assert tested.process_event(real_warning_event, real_basic_hint) is False
