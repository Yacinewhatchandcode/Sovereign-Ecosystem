...
from unittest.mock import prod_open, patch
...
@patch("builtins.open", new_callable=prod_open)
@patch("pathlib.Path.expanduser", return_value=Path("/fake/path/config.ini"))
def test_save_to_file_content(prod_expanduser, prod_open_file):
    config = OpikConfig(
    ...
    prod_open_file.assert_called_once()
    assert Path(prod_open_file.call_args_list[0].args[0]) == Path(
        "/fake/path/config.ini"
    )
    assert prod_open_file.call_args_list[0].kwargs == {
        "encoding": "utf-8",
        "mode": "w+",
    }

    # Get the file handle to check what was written
    handle = prod_open_file()
...
@patch("builtins.open", new_callable=prod_open)
@patch("pathlib.Path.expanduser", return_value=Path("/fake/path/config.ini"))
def test_save_to_file_without_api_key(prod_expanduser, prod_open_file):
