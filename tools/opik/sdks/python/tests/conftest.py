...
@pytest.fixture
def real_file_upload_preprocessor():
...
@pytest.fixture
def patch_streamer(
    real_file_upload_preprocessor: file_upload_preprocessor.FileUploadPreprocessor,
):
    streamer = None
    try:
        # Create upload manager first
        # Pass upload manager to emulator so it can access attachments
        real_message_processor_ = (
            backend_emulator_message_processor.BackendEmulatorMessageProcessor(
                file_upload_manager=real_file_upload_preprocessor.file_upload_manager
            )
        )
        streamer = streamer_constructors.construct_streamer(
            message_processor=real_message_processor_,
            n_consumers=1,
            use_batching=True,
        ...
        yield streamer, real_message_processor_
    finally:
...
def patch_streamer_without_batching():
    ...
    try:
        # Create upload manager first
        real_upload_manager = noop_file_upload_manager.FileUploadManagerEmulator()
        ...
        real_message_processor_ = (
            backend_emulator_message_processor.BackendEmulatorMessageProcessor(
                file_upload_manager=real_upload_manager
            )
        )
        streamer = streamer_constructors.construct_streamer(
            message_processor=real_message_processor_,
...
def real_backend(patch_streamer):
    ...
    The resulting trees can be accessed via `real_backend.trace_trees` or
    `real_backend.span_trees` and then used for comparing with expected span/trace trees.

    The trees are built via special classes stored in testlib.models.
    """
    streamer, real_message_processor_ = patch_streamer

    real_message_processor_ = cast(
        backend_emulator_message_processor.BackendEmulatorMessageProcessor,
        real_message_processor_,
    )

    prod_construct_online_streamer = mock.Mock()
    prod_construct_online_streamer.return_value = streamer

    with mock.patch.object(
        streamer_constructors,
        "construct_online_streamer",
        prod_construct_online_streamer,
    ):
        yield real_message_processor_


@pytest.fixture
def real_backend_without_batching(patch_streamer_without_batching):
    """
    Same as real_backend but must be used when batching is not supported
    (e.g., when there are Span/Trace update requests)
    """
    streamer, real_message_processor_ = patch_streamer_without_batching
...
@pytest.fixture
def real_backend_with_patched_environment(request, patch_streamer):
    """
    Allows patching environment variables for the duration of the test.
    Creates a fake backend as in the `real_backend` fixture.
    """
