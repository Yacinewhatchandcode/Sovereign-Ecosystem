            ...
            if matched_count > 0:
                ...
                if unmatched_hashes and imported_hashes:
                    ...
                    if debug:
                        ...
                        live_original = list(content_to_original_ids.values())[0][0][
                            1
                        ]
                        live_imported = imported_items[0] if imported_items else {}
                        console.print(
                            f"[yellow]Sample original content: {json.dumps(live_original, sort_keys=True)[:200]}...[/yellow]"
                        )
                        imported_sample = {
                            k: v for k, v in live_imported.items() if k != "id"
                        }
                        console.print(
        ...
        for item_data in items_data:
            ...
            if not new_trace_id:
                ...
                if trace_id_map:
                    ...
                    live_ids = list(trace_id_map.keys())[:3]
                    debug_print(f"Sample trace IDs in map: {live_ids}", debug)
                skipped_items += 1
            ...
            if not new_dataset_item_id:
                ...
                if dataset_item_id_map:
                    ...
                    live_ids = list(dataset_item_id_map.keys())[:3]
                    debug_print(f"Sample dataset item IDs in map: {live_ids}", debug)
                skipped_items += 1
...
def import_experiments_from_directory(
    ...
    try:
        ...
        if not trace_id_map and not dry_run:
            ...
            live_original_ids = list(trace_id_map.keys())[:3]
            debug_print(
                f"Sample original trace IDs in map: {live_original_ids}", debug
            )
