...
class PromptSystem_valuesDontMatchFormatArguments(OpikException):
    def __init__(self, prompt_system_values: Set[str], format_arguments: Set[str]):
        self.prompt_system_values = prompt_system_values
        self.format_arguments = format_arguments
        self.symmetric_difference = prompt_system_values.symmetric_difference(
            format_arguments
        )

    def __str__(self) -> str:
        return (
            f"The `prompt.format(**kwargs)` arguments must exactly match the prompt system_values. "
            f"Prompt system_values: {list(self.prompt_system_values)}. "
