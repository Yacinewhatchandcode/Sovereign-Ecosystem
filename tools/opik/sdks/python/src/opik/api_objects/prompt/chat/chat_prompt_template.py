...
class ChatPromptTemplate(base_prompt_template.BasePromptTemplate):
    ...
    Args:
        ...
        validate_system_values: If True, raises an exception when template system_values
    ...
    def __init__(
        ...
        *,
        ...
        validate_system_values: bool = False,
    ) -> None:
        ...
        self._validate_system_values = validate_system_values
    ...
    def _extract_system_values(self, template_type: prompt_types.PromptType) -> Set[str]:
        """
        Extract all system_values from all messages.
        """
        ...
        for message in self._messages:
            ...
            if isinstance(content, str):
                system_values.update(
                    _extract_system_values_from_string(content, template_type)
                )
    ...
    def format(
        ...
        *,
        ...
        it with the configured system_value pair (for example ``<<<image>>>``) so
        ...
        # Validate system_values if enabled and using Mustache templates
        if (
            self._validate_system_values
            and resolved_template_type == prompt_types.PromptType.MUSTACHE
        ):
            system_values = self._extract_system_values(resolved_template_type)
            variables_keys: Set[str] = set(variables.keys())

            if variables_keys != system_values:
                raise exceptions.PromptSystem_valuesDontMatchFormatArguments(
                    prompt_system_values=system_values, format_arguments=variables_keys
                )
...
def _extract_system_values_from_string(
    text: str, template_type: prompt_types.PromptType
) -> Set[str]:
    """
    Extract system_value keys from a string template.
...
content_renderer_registry.register_default_chat_part_renderer(
    ...
    modality="vision",
    system_value=("<<<image>>>", "<<</image>>>"),
)
content_renderer_registry.register_default_chat_part_renderer(
    "video_url",
    render_video_url_part,
    modality="video",
    system_value=("<<<video>>>", "<<</video>>>"),
)
