...
class ChatContentRendererRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._modality_system_values: MutableMapping[
            prompt_types.ModalityName, Tuple[str, str]
        ] = {
            "vision": ("<<<image>>>", "<<</image>>>"),
            "video": ("<<<video>>>", "<<</video>>>"),
        }
        self._default_system_value: Tuple[str, str] = ("<<<media>>>", "<<</media>>>")
        self._system_value_value_limit = 500
    ...
    def register_part_renderer(
        ...
        *,
        ...
        system_value: Optional[Tuple[str, str]] = None,
    ) -> None:
        """
        ...
        if modality and system_value:
            self._modality_system_values[modality] = system_value
    ...
    def _flatten_parts_to_text(
        ...
        for part in parts:
            ...
            if modality and modality_flags.get(modality, False):
                ...
                continue
            if modality and modality in self._modality_system_values:
                prefix, suffix = self._modality_system_values[modality]
            else:
                prefix, suffix = self._default_system_value
            system_value_value = self._extract_system_value_value(part)
            if system_value_value:
                truncated = self._truncate_system_value_value(system_value_value)
                segments.append(f"{prefix}{truncated}{suffix}")
        return "\n\n".join(segment for segment in segments if segment)

    @staticmethod
    def _extract_system_value_value(part: prompt_types.ContentPart) -> str:
        part_type = part.get("type", "").lower()
    ...
    def _truncate_system_value_value(self, value: str) -> str:
        if len(value) <= self._system_value_value_limit:
            return value
        if self._system_value_value_limit <= 3:
            return value[: self._system_value_value_limit]
        return value[: self._system_value_value_limit - 3] + "..."
