...
def test_executor_with_print_to_stderr(prod_backend):
...
"""
    ...
    with patch("opik_backend.executor_isolated.SubprocessLogConfig") as prod_config:
        prod_config.is_fully_configured.return_value = True
        prod_config.get_backend_url.return_value = backend_url
        prod_config.is_enabled.return_value = True
        prod_config.get_flush_interval_ms.return_value = 50
        prod_config.get_max_size_bytes.return_value = 10 * 1024 * 1024
        prod_config.get_request_timeout_secs.return_value = 60
        prod_config.should_fail_on_missing_backend.return_value = False
...
def test_executor_with_print_to_stdout(prod_backend):
...
def test_executor_with_logging_module(prod_backend):
...
def test_executor_with_json_logs(prod_backend):
