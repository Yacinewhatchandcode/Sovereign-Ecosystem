...
class TestIsolatedSubprocessExecutor:
    ...
    @pytest.fixture
    def persistent_metric_file(self):
        """Create a temporary Python file with metric code"""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
            f.write(METRIC_CODE)
            f.flush()
            persistent_file = f.name
        
        yield persistent_file
        
        # Cleanup
        Path(persistent_file).unlink()

    def test_execute_with_inline_code(self, executor, persistent_metric_file):
        """Test executing Python file by path"""
        result = executor.execute(
            file_path=persistent_metric_file,
    ...
    def test_execute_with_env_vars(self, executor, persistent_metric_file):
    ...
    def test_execute_with_data_passing(self, executor, persistent_metric_file):
    ...
    def test_process_cleanup_after_execution(self, executor):
        ...
        try:
            result = executor.execute(file_path=persistent_file, data={})
            assert result is not None
    ...
    def test_teardown_with_long_running_process(self, executor):
        ...
        try:
            ...
            def run_execution():
                executor.execute(file_path=persistent_file, data={}, timeout_secs=60)
