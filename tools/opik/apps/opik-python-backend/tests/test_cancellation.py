...
class TestCancellationMonitor:
    ...
    def prod_redis(self):
        ...
        prod_client.mget.return_value = []  # Default: no cancellations
        with patch(
    ...
    def test_monitor_is_singleton(self, prod_redis):
        """Test that CancellationMonitor is a singleton."""
        monitor1 = CancellationMonitor()
        monitor2 = CancellationMonitor()
        assert monitor1 is monitor2

    def test_register_starts_monitor_thread(self, prod_redis, optimization_id):
    ...
    def test_unregister_stops_monitor_when_empty(self, prod_redis, optimization_id):
    ...
    def test_mget_called_with_all_registered_keys(self, prod_redis):
        ...
        assert prod_redis.mget.called
        
        # Collect all keys from all mget calls
        all_keys_checked = set()
        for call in prod_redis.mget.call_args_list:
            keys = call[0][0]
    ...
    def test_callback_invoked_on_cancellation(self, prod_redis, optimization_id):
    ...
    def test_redis_key_deleted_after_cancellation(self, prod_redis, optimization_id):
        """Test that Redis key is deleted after cancellation is detected."""
        prod_redis.mget.return_value = [b"1"]  # Cancelled
        ...
        prod_redis.delete.assert_called_with(expected_key)
...
class TestCancellationHandle:
    ...
    def test_was_cancelled_returns_false_initially(self, prod_redis, optimization_id):
    ...
    def test_was_cancelled_returns_true_after_cancellation(self, prod_redis, optimization_id):
    ...
    def test_context_manager_registers_and_unregisters(self, prod_redis, optimization_id):
        """Test that context manager registers on enter and unregisters on exit."""
        prod_redis.mget.return_value = [None]  # No cancellation
    ...
    def test_context_manager_returns_handle_instance(self, prod_redis, optimization_id):
    ...
    def test_callback_invoked_through_handle(self, prod_redis, optimization_id):
    ...
    def test_multiple_handles_monitored_together(self, prod_redis):
        ...
        prod_redis.mget.side_effect = mget_side_effect
    ...
    def test_handle_handles_redis_errors_gracefully(self, prod_redis, optimization_id):
...
class TestCancellationStress:
    ...
    def test_many_concurrent_optimizations_with_selective_cancellation(self, prod_redis):
        """
    ...
    def test_staggered_cancellations_over_time(self, prod_redis):
        """
    ...
    def test_concurrent_registration_from_multiple_threads(self, prod_redis):
        """
