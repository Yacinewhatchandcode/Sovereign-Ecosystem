...
def test_validate_with_no_matched_topics__restrict_mode__validation_passed():
    prod_pipeline = mock.Mock()
    prod_pipeline.return_value = {
        "labels": ["finance", "healthcare", "politics"],
        "scores": [0.3, 0.2, 0.1],
    }

    topic_validator = validator.TopicValidator(pipeline=prod_pipeline)
...
def test_validate_with_custom_threshold__restrict_mode__validation_failed():
    prod_pipeline = mock.Mock()
    prod_pipeline.return_value = {"labels": ["healthcare"], "scores": [0.35]}
