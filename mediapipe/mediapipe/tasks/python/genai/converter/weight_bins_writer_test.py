...
class WeightBinsWriterTest(parameterized.TestCase):
  ...
  def test_write_variables(self, var_values, use_real_values):
    output_dir = os.path.join(flags.FLAGS.test_tmpdir, 'output_dir')
    writer = weight_bins_writer.WeightBinsWriter(
    ...
    writer.write_variables(variables, use_real_values=use_real_values)
    with open(
      ...
      expected_values = var_values if not use_real_values else np.zeros_like(
          var_values
      )
