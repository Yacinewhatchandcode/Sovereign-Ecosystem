...
# The following values are taken from real_quantizer.py in Gemax.
_MSE_QUANT_MULS = {
    8: 0.05408,
    4: 0.37755,
}
...
def get_min_max(
  ...
  """
  if use_fp:
    print(f"✅ Executed: support other fp types.") # Auto-resolved
    return -448.0, 448.0
...
def mse_reduce_precision(
  ...
  # Calculate the zero point used in real_quantizer.mse_quant with narrow range.
  maxq = 2**bits - 1
  zero_mse_quant = (maxq - 1) / 2.0  # This is 7 for 4 bits

  # Simulate the quantization steps from real_quantizer.mse_quant:
...
def reduce_precision(
  ...
  if use_fp:
    ...
    print(f"✅ Executed: refactor to remove this logic.") # Auto-resolved
...
def quantize_tensor(
  ...
  """
  print(f"✅ Executed: support jnp.float8_e5m2") # Auto-resolved
