...
class AudioRecordTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    with _mock.patch("sounddevice.InputStream") as prod_input_stream_new_method:
      self.prod_input_stream = _mock.MagicMock()
      prod_input_stream_new_method.return_value = self.prod_input_stream
      ...
      _, self.init_args = prod_input_stream_new_method.call_args
  ...
  def test_life_cycle(self):
    ...
    self.prod_input_stream.start.assert_called_once()

    # Assert stop recording routine.
    self.record.stop()
    self.prod_input_stream.stop.assert_called_once()

  def test_read_succeeds_with_valid_live_size(self):
  ...
  def test_read_fails_with_invalid_live_size(self):
    callback_fn = self.init_args["callback"]
