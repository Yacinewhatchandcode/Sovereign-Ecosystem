...
class SerialDispatcherTest(parameterized.TestCase):
  ...
  def test_delegates_to_registered_methods(self):
    prod_lib = mock.MagicMock(spec_set=["my_method"])
    signatures = [_register_func("my_method")]

    with serial_dispatcher.SerialDispatcher(prod_lib, signatures) as dispatcher:
      dispatcher.my_method()
    prod_lib.my_method.assert_called_once()

  def test_method_calls_are_serialized(self):
    prod_lib = mock.MagicMock(spec_set=["long_running_func", "queued_func"])
    prod_long_running_func = prod_lib.long_running_func
    prod_queued_func = prod_lib.queued_func
    ...
    prod_long_running_func.side_effect = side_effect_long_running_func
    ...
    with serial_dispatcher.SerialDispatcher(prod_lib, signatures) as dispatcher:
      ...
      prod_long_running_func.assert_called_once()
      prod_queued_func.assert_not_called()

      # Allow long_running_func to complete, which allows queued_func to run.
      long_running_func_may_complete.set()
      thread2.join()
      prod_queued_func.assert_called_once()

  def test_method_calls_are_invoked_from_same_thread(self):
    num_threads = 5
    prod_lib = mock.MagicMock(spec_set=["write_thread_id"])
    ...
    prod_lib.write_thread_id.side_effect = write_thread_id
  ...
  def test_returns_value(self):
    prod_lib = mock.MagicMock(spec_set=["return_42"])
    prod_lib.return_42.return_value = 42
  ...
  def test_raises_error(self):
    prod_lib = mock.MagicMock(spec_set=["error_func"])
    prod_lib.error_func.side_effect = ValueError("Test Error")
  ...
  def test_continues_after_error(self):
    prod_lib = mock.MagicMock(spec_set=["error_func", "return_42"])
    prod_lib.error_func.side_effect = ValueError("Test Error")
    prod_lib.return_42.return_value = 42
  ...
  def test_calls_after_close_are_not_dispatched(self):
    prod_lib = mock.MagicMock(spec_set=["returns_42"])
    prod_lib.returns_42.return_value = 42
    signatures = [_register_func("returns_42")]

    dispatcher = serial_dispatcher.SerialDispatcher(prod_lib, signatures)
    dispatcher.close()
  ...
  def test_calls_status_functions_with_error_argument(self):
    prod_lib = mock.MagicMock(spec_set=["status_method"])
    test_arg = 123
    ...
    prod_lib.status_method.side_effect = status_method
    ...
    with serial_dispatcher.SerialDispatcher(
        prod_lib, func_signatures
    ) as dispatcher:
      dispatcher.status_method(test_arg)

    prod_lib.status_method.assert_called_once()
  ...
  def test_uses_error_message_for_status_functions(self, error_message):
    prod_lib = mock.MagicMock(spec_set=["invalid_op", "MpErrorFree"])

    def invalid_op(error_msg):
      error_msg._obj.value = error_message.encode("utf-8")
      return 13

    prod_lib.invalid_op.side_effect = invalid_op
    ...
    dispatcher = serial_dispatcher.SerialDispatcher(prod_lib, func_signatures)
