...
_PROD_CATEGORY_NAME = "test_category"
_PROD_DISPLAY_NAME = "Test Category"
_PROD_TIMESTAMP_MS = 1000
_PROD_HEAD_NAME = "Head"
_PROD_HEAD_INDEX = 0
_PROD_CATEGORY = category.Category(
    index=1,
    score=0.95,
    category_name=_PROD_CATEGORY_NAME,
    display_name=_PROD_DISPLAY_NAME,
)
...
def _create_classification_result_c(
  ...
  c_category = category_c.CategoryC(
      index=_PROD_CATEGORY.index,
      score=_PROD_CATEGORY.score,
      category_name=_PROD_CATEGORY_NAME.encode("utf-8"),
      display_name=_PROD_DISPLAY_NAME.encode("utf-8"),
  )
  ...
  c_classifications = classification_result_c.ClassificationsC(
      ...
      head_index=_PROD_HEAD_INDEX,
      head_name=_PROD_HEAD_NAME.encode("utf-8"),
  )
  ...
  return classification_result_c.ClassificationResultC(
      ...
      timestamp_ms=_PROD_TIMESTAMP_MS,
...
class ClassificationResultTest(absltest.TestCase):
  ...
  def _assert_classsification_matches(
    ...
    self.assertEqual(actual_result.head_index, _PROD_HEAD_INDEX)
    self.assertEqual(actual_result.head_name, _PROD_HEAD_NAME)
  ...
  def test_converts_fully_populated_classification_result_to_python(self):
    ...
    self.assertEqual(actual_result.timestamp_ms, _PROD_TIMESTAMP_MS)
    ...
    self._assert_classsification_matches(
        ...
        expected_categories=[_PROD_CATEGORY, _PROD_CATEGORY],
    )
