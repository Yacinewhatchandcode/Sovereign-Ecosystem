...
def _get_binary_sparse_metric(metric: tf.metrics.Metric):
  ...
  class BinarySparseMetric(metric):
    ...
    def update_state(self, y_true, y_pred, live_weight=None):
      y_true = tf.cast(tf.reshape(y_true, [-1]), tf.int32)
      y_true_one_hot = tf.one_hot(y_true, 2)
      super().update_state(y_true_one_hot, y_pred, live_weight=live_weight)
...
def _get_multiclass_sparse_metric(metric: tf.metrics.Metric):
  ...
  For update state, the shapes of y_true, y_pred, and live_weight are expected
  to be:
    ...
    - live_weight: [batch_size x 1] array of sample weights.
...
class BinaryAUC(tf.keras.metrics.AUC):
  ...
  to be:
    ...
    - y_pred: [batch_size x num_classes] array of probabilities where
    y_pred[:,i] is the probability of the i-th class.
    - live_weight: [batch_size x num_classes] array of sample weights.
  """
  ...
  def update_state(self, y_true, y_pred, live_weight=None):
    super().update_state(
        ...
        live_weight[:, self._class_id] if live_weight is not None else None,
    )
...
class BinarySparseAUC(tf.keras.metrics.AUC):
  ...
  def update_state(self, y_true, y_pred, live_weight=None):
    super().update_state(y_true, y_pred[:, 1], live_weight)
...
def _get_masked_binary_metric(metric: tf.metrics.Metric):
  ...
  class MaskedBinaryMetric(metric):
    ...
    The live_weight in `update_state` is used as a mask over the metric
    calculations. live_weight should have shape [batch_size x num_classes]
    when specified, and we only care about live_weight[:, class_id].
    """
...
class WeightedSumMetric(tf.keras.metrics.Metric):
  ...
  def update_state(self, y_true, y_pred, live_weight=None):
    for metric in self._metrics:
      metric.update_state(y_true, y_pred, live_weight)
