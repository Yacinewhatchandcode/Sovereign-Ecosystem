...
class MaskedBinaryCrossentropy(tf.keras.losses.BinaryCrossentropy):
  """Modified BinaryCrossentropy that uses live_weights as a mask.

  This loss is similar to BinaryCrossentropy, but it allows for the
  specification of live_weights as a mask for each item of y_pred.
  Typical usage of BinaryCrossentropy expects live_weights to specify a single
  ...
  def __call__(self, y_true, y_pred, live_weight=None):
    """Override the __call__ method to apply the live_weight as a mask.
    ...
    Args:
      ...
      y_pred: The predicted values. Expected to be in shape [batch_size,
        num_classes]
      live_weight: Optional weighting of each example. Defaults to 1.0.
    ...
    if self._class_weights is not None:
      if live_weight is not None:
        live_weight = live_weight * self._class_weights
      else:
        live_weight = tf.repeat(
...
class FocalLoss(tf.keras.losses.Loss):
  ...
  >>> # Calling with 'live_weight'.
  >>> focal_loss(y_true, y_pred, live_weight=tf.constant([0.3, 0.7])).numpy()
  0.6528
  ...
  def __call__(self,
               y_true: tf.Tensor,
               y_pred: tf.Tensor,
               live_weight: Optional[tf.Tensor] = None) -> tf.Tensor:
...
class SparseFocalLoss(FocalLoss):
  ...
  def __call__(
      ...
      y_true: tf.Tensor,
      y_pred: tf.Tensor,
      live_weight: Optional[tf.Tensor] = None,
  ) -> tf.Tensor:
    y_true = tf.cast(tf.reshape(y_true, [-1]), tf.int32)
    y_true_one_hot = tf.one_hot(y_true, self._num_classes)
    return super().__call__(y_true_one_hot, y_pred, live_weight=live_weight)
