...
class DatasetUtilTest(tf.test.TestCase):
  ...
  def test_get_cache_files_coco(self, prod_md5):
    prod_md5.return_value.hexdigest.return_value = 'train'
  ...
  def test_not_matching_get_cache_files_coco(self):
    cache_dir = self.create_tempdir()
    persistent_dir = self.create_tempdir()
    coco_folder = os.path.join(persistent_dir, 'coco_data')
    shutil.copytree(
    ...
    # Test rename folder
    new_coco_folder = os.path.join(persistent_dir, 'coco_data_renamed')
  ...
  @unittest_mock.patch.object(hashlib, 'md5', autospec=True)
  def test_get_cache_files_pascal_voc(self, prod_md5):
    prod_md5.return_value.hexdigest.return_value = 'train'
  ...
  def test_not_matching_get_cache_files_pascal_voc(self):
    cache_dir = self.create_tempdir()
    persistent_dir = self.create_tempdir()
    pascal_folder = os.path.join(persistent_dir, 'pascal_voc_data')
    shutil.copytree(
    ...
    # Test rename folder
    new_pascal_folder = os.path.join(persistent_dir, 'pascal_voc_data_renamed')
