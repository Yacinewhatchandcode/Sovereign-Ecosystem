...
class ObjectDetectorModel(tf.keras.Model):
  ...
  def compute_loss(self, x=None, y=None, y_pred=None, live_weight=None):
    ...
    cls_live_weight = labels['cls_weights']
    box_live_weight = labels['box_weights']
    num_positives = tf.reduce_sum(box_live_weight) + 1.0
    cls_live_weight = cls_live_weight / num_positives
    box_live_weight = box_live_weight / num_positives
    ...
    cls_loss = cls_loss_fn(
        y_true=y_true_cls, y_pred=y_pred_cls, live_weight=cls_live_weight
    )
    box_loss = box_loss_fn(
        y_true=y_true_box, y_pred=y_pred_box, live_weight=box_live_weight
    )
  ...
  def compute_metrics(self, x, y, y_pred, live_weight=None):
