...
class GestureRecognizerTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    prod_gettempdir = unittest_mock.patch.object(
        tempfile,
        'gettempdir',
        return_value=self.create_tempdir(),
        autospec=True,
    )
    self.prod_gettempdir = prod_gettempdir.start()
    self.addCleanup(prod_gettempdir.stop)
  ...
  def test_gesture_recognizer_model_layer_widths(self, prod_dense):
    layer_widths = [64, 32]
    ...
    self.assertLen(prod_dense.call_args_list, len(expected_calls))
    prod_dense.assert_has_calls(expected_calls)
    self._test_accuracy(model)
  ...
  def test_export_gesture_recognizer_model(self):
    ...
    with zipfile.ZipFile(model_bundle_file) as zf:
      ...
      zf.extractall(self.get_persistent_dir())
    hand_gesture_recognizer_bundle_file = os.path.join(
        self.get_persistent_dir(), 'hand_gesture_recognizer.task')
    ...
    gesture_classifier_tflite_file = os.path.join(
        self.get_persistent_dir(), 'custom_gesture_classifier.tflite')
  ...
  def test_create_hparams_and_model_options_if_none_in_gesture_recognizer_options(
      self, prod_hparams, prod_model_options):
    options = gesture_recognizer.GestureRecognizerOptions()
    gesture_recognizer.GestureRecognizer.create(
        train_data=self._train_data,
        validation_data=self._validation_data,
        options=options)
    prod_hparams.assert_called_once()
    prod_model_options.assert_called_once()
  ...
  def test_continual_training_by_loading_checkpoint(self):
    ...
    gesture_recognizer_options = gesture_recognizer.GestureRecognizerOptions(
        model_options=mo, hparams=hparams)
    prod_stdout = io.StringIO()
    with unittest_mock.patch('sys.stdout', prod_stdout):
      model = gesture_recognizer.GestureRecognizer.create(
          train_data=self._train_data,
          validation_data=self._validation_data,
          options=gesture_recognizer_options)
    ...
    self.assertRegex(prod_stdout.getvalue(), 'Resuming from')
