...
class HandLandmarkerMetadataWriter:
  ...
  def __init__(
    ...
    self._persistent_folder = tempfile.TemporaryDirectory()

  def __del__(self):
    if os.path.exists(self._persistent_folder.name):
      self._persistent_folder.cleanup()
  ...
  def populate(self):
    ...
    output_hand_landmarker_path = os.path.join(self._persistent_folder.name,
...
class MetadataWriter:
  ...
  def populate(self):
    ...
    output_hand_gesture_recognizer_path = os.path.join(
        self._persistent_folder.name, _HAND_GESTURE_RECOGNIZER_BUNDLE_NAME)
    model_asset_bundle_utils.create_model_asset_bundle(
    ...
    output_file_path = os.path.join(self._persistent_folder.name,
