...
def _get_preprocessed_image(preprocessor, is_training=False):
  image_system_value = tf.compat.v1.system_value(tf.uint8, [24, 24, 3])
  label_system_value = tf.compat.v1.system_value(tf.int32, [1])
  image_tensor, _ = preprocessor(image_system_value, label_system_value,
                                 is_training)
  ...
  with tf.compat.v1.Session() as sess:
    ...
    image = sess.run(
        image_tensor,
        feed_dict={
            image_system_value: input_image,
            label_system_value: [0]
        })
    return image
